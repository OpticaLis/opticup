...
| 42 | 2026-05-24 | WhatsApp blocker diagnosis: two-gate model | WABA onboarding has 2 independent gates: (1) Meta Business Verification (business-level, no BSP bypass, document-based), (2) phone number registration (fresh 058-688-3168 solves this gate entirely). Prior failures undiagnosed — Gate 1 or Gate 2? FIRST STEP UPDATED: Daniel checks Meta Business Verification status at business.facebook.com before paying for Dialog360. |
| 41 | 2026-05-24 | 3-way WhatsApp comparison: Dialog360 confirmed over Make Cloud API + Direct Cloud API | Decisive factor: smb_message_echoes (BSP-exclusive) = complete two-way inbox with staff phone coexistence. Make = dead end for M12 inbox. Direct Meta = staff reply blind spots. $59-119/mo is negligible vs operational value. M12 Brief decision stands. First step: Dialog360 account signup + WABA number onboarding + first template submission THIS WEEK. |
| 40 | 2026-05-24 | WhatsApp re-plan: Dialog360 replaces Green API (Daniel hard correction) | Green API = banned at 1K+ marketing msgs (ToS). Dialog360 (official WABA) locked per M12 Brief. SPEC 6 split: 6A infra (direct EF→360dialog, channel_configs table, consent) + 6B templates+launch (blocked on Meta template approval). Template submission = longest lead time, start NOW. |
| 39 | 2026-05-24 | WhatsApp initial assessment (superseded by #40) | Green API via Make recommendation — wrong, contradicted sealed M12 Brief. Daniel corrected. |
| 38 | 2026-05-24 | M4 Scale Readiness Master Plan authored | 10K-lead projection: 5 sequenced SPECs (queue lifecycle → throughput → screen audit → email vendor → list hygiene). Queue cleanup only targets `sent` (54% cancelled rows accumulate forever = real bloat). SPECs 1-3 pre-10K-blocking; 4-5 can wait. Timeline question surfaced to Daniel. |
...
→ Full detail: [`decisions/M4.md`](decisions/M4.md)

| # | Date | Topic | One-liner |
|---|---|---|---|
| 1 | 2026-05-24 | Scale Readiness Master Plan | 10K-lead infrastructure: 5 SPECs sequenced (queue lifecycle, throughput, screen audit, email vendor, list hygiene). Key finding: cleanup cron ignores cancelled rows (54% of table). |
| 2 | 2026-05-24 | WhatsApp channel assessment (initial, superseded by #3) | Initial Green API recommendation — WRONG, contradicted sealed M12 Brief. |
| 3 | 2026-05-24 | WhatsApp re-plan: Dialog360 (WABA) | Daniel hard correction: Green API = banned at 1K+. Dialog360 locked (aligns with M12 Brief). SPEC 6 split into 6A (infra) + 6B (templates+launch). Template approval is longest lead time — start NOW. 5 blocking Qs surfaced. |
| 4 | 2026-05-24 | 3-way WhatsApp path comparison: Dialog360 CONFIRMED | Dialog360 vs Make Cloud API vs Direct Cloud API. Decisive factor: smb_message_echoes (BSP-only) gives complete two-way inbox with staff phone coexistence. Make = dead end for inbox. Direct = blind spot on staff replies. $59-119/mo is the cost of zero blind spots. M12 Brief decision stands. |
| 5 | 2026-05-24 | WhatsApp blocker diagnosis: two independent gates | Gate 1 = Meta Business Verification (business-level, BSP can't bypass). Gate 2 = phone number registration (fresh 058-688-3168 solves this). Must check Gate 1 status FIRST — if not verified, THAT is the real blocker, not the phone. Action: Daniel checks business.facebook.com → Security Center → Business verification status. |
