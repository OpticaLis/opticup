...
| 39 | 2026-05-24 | WhatsApp channel: separate SPEC 6, not bundled with scale | Daniel directive "I must send WhatsApp." Code audit: dispatch is webhook-based (EF → Make → vendor), adding WhatsApp = validation lift + Make route config. Green API via Make first. 3 blocking Qs surfaced. Runs parallel with SPEC 3. |
...
| 2 | 2026-05-24 | WhatsApp channel assessment | Separate track (SPEC 6), not bundled with scale. Dispatch architecture is webhook-based — adding WhatsApp is a validation/schema lift, not re-architecture. Green API via Make first; 3 blocking questions surfaced to Daniel. |
