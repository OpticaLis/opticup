...
- **SQL migration files — Iron Rule 32 hook comment-awareness (added 2026-05-15 from SECURITY_HOTFIX_3 P-EXEC-2).** When writing migration `.sql` files, AVOID destructive-pattern keywords (`DROP`, `DELETE`, `TRUNCATE`, `REVOKE`) inside `--` SQL comments. The Iron Rule 32 hook (`scripts/checks/destructive-ops-declared.mjs`) scans staged diffs for these patterns and does not distinguish active SQL from comments — a comment that says "Rollback: DROP POLICY ..." will block the commit. Reference the backup folder (`modules/Module N/backups/{TS}_{SLUG}/`) for rollback recipes instead of inlining them in migration-file comments. Long-term fix tracked as `IRON_RULE_32_HOOK_COMMENT_AWARENESS` SPEC. Source: `SECURITY_HOTFIX_3_2026_05_15/EXECUTION_REPORT.md` D-3 + FINDINGS F-6.
- **Block A demo tests for hardened RPCs (added 2026-05-15 from SECURITY_HOTFIX_3 P-EXEC-1).** When a SPEC §1.5 (or equivalent) hardens SECURITY DEFINER RPCs with a 3-role-aware Block A (per `JWT_VALIDATION_HEADER.sql`), validate post-migration via the canonical DO-block in `.claude/skills/opticup-executor/references/BLOCK_A_DEMO_TESTS.sql`. The reference file includes the wrong-tenant JWT test (expects 42501), the service_role bypass test (expects no 42501), and the gotchas around empty-string JWT (22P02 parse error — test setup artifact, NOT a real anon path). Do NOT hand-roll the test pattern — past SPECs have lost ~5 minutes per RPC chasing the empty-JWT JSON-cast trap. Source: `SECURITY_HOTFIX_3_2026_05_15/EXECUTION_REPORT.md` §7 + P-EXEC-1.
