...
**SPEC-shape patterns (codified 2026-05-20 from `M4_SHORT_LINKS_400_FIX` P-AUTHOR-1 + `M4_SMS_RATE_LIMIT_HOTFIX_2026_05_20` P-AUTHOR-1):**

- **Diagnosis-driven SPECs.** When an upstream investigation/audit report has already done the diagnostic work, analyzed alternative options, and recommended a course of action, the SPEC's §0 should NOT re-derive the analysis. Cite the investigation report (path + commit hash), bind scope to the recommended option, and use the SPEC for execution-shape decisions (steps, success criteria, destructive-ops declaration). This shortens the author cycle from ~60 min to ~15 min when the diagnostic legwork is already done.

- **P0 hotfix shape — Light Pipeline + same-thread Foreman-as-Executor.** For urgency-class P0 incidents (live data corruption, customer-facing outage, SMS-vendor rate-limit storm, payment-flow break), the SPEC can run end-to-end in ONE chat: Foreman authors SPEC → same chat executes the steps → same chat writes EXECUTION_REPORT + FOREMAN_REVIEW. NO sub-agent spawn, NO Reviewer/Localhost-Tester hand-off. The Light Pipeline is the explicit opt-out from the 5-hat Pipeline for incidents where the 1-2h overhead of full Pipeline would mean an extra hour of customer impact. Document the urgency-class choice in §1 Goal and the rationale (which Pipeline hat was skipped + why). Precedent: `M4_SMS_RATE_LIMIT_HOTFIX_2026_05_20` (30-min wall-clock SPEC→close during a live customer-impacting incident).
...
5.4. **Runtime semantics rehearsal — DB-touching SPECs only (MANDATORY when SPEC adds/modifies function headers, RLS USING clauses, view flags, or grants; added 2026-05-15 from SECURITY_HOTFIX_2 Author Proposal P-AUTHOR-2).** Name collisions are not the only failure class — *behavior* collisions kill SPECs at execution time. Before sealing the SPEC, rehearse runtime semantics in §0 Pre-Authoring Reality Check:
   ...
   - **§5.4a Enum-distribution probe — generalized from status-column probe (added 2026-05-20 from `M4_SHORT_LINKS_DASHBOARD_REDESIGN` P-AUTHOR-1).** The status-column rule above generalizes to ALL typed columns the SPEC's logic depends on. For every column the SPEC's §0 references with a literal value (`status='X'`, `link_type='Y'`, `payment_status='Z'`, `channel='sms'`, …), the rehearsal MUST query `SELECT <col>, count(*) FROM <table> GROUP BY <col> ORDER BY count DESC` for the SPEC's tenant scope. If the literal value has 0 rows OR is rare (<5% of total), the SPEC declares the cardinality in §0 and decides up-front whether the implementation expects empty-set behavior or escalates. Saves the executor from the "filter returns 0 rows but the data looks healthy" stop-trigger.

   - **§5.4b Brief data-drift table — explicit drift acknowledgment (added 2026-05-20 from `M4_SHORT_LINKS_DASHBOARD_REDESIGN` P-AUTHOR-2).** When the SPEC's source Brief contains row counts, ratios, or column-value examples, the SPEC's §0 Pre-Authoring Reality Check MUST include a 3-column drift table (Brief value | Live value | Drift class: STRUCTURAL/STYLISTIC/INFORMATIONAL). STRUCTURAL drift (e.g., enum-value naming mismatch) BLOCKS sealing until resolved. STYLISTIC drift (e.g., 1,179 vs ~1,200) is logged and proceeds. Without this table the SPEC sails on stale data and the executor catches drift mid-run.

   - **§5.4c Column-existence probe — every cited JOIN / SELECT projection (added 2026-05-20 from `M4_SHORT_LINKS_DASHBOARD_REDESIGN` P-AUTHOR-3).** Mandatory for any SPEC whose §0 includes a JOIN, `.select('a, b, c')` projection, or `c.<col>` reference. Run `SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='<X>'` for EVERY table the implementation will read, and explicitly tick off each cited column. Today's regression cost Daniel 1 round of live verification because the SPEC referenced `short_link_clicks.lead_id` which doesn't exist — the column lives on `short_links` instead.

   - **§5.4d PostgREST 1000-row cardinality probe (added 2026-05-20 from `M4_SHORT_LINKS_DASHBOARD_REDESIGN` P-AUTHOR-4).** PostgREST's default 1000-row response limit is a per-query failure class. The SPEC §0 MUST estimate result-row cardinality for every `.select(...)` it specifies and choose a query shape accordingly. Three Tiers: (T1) `<1000` rows expected → standalone fetch is fine. (T2) `1000-10000` → embedded-JOIN (PostgREST embeds bypass the limit on the child side, parent stays bounded) OR explicit chunked pagination. (T3) `>10000` → server-side aggregate (RPC) or cron-rebuilt materialized view, never a single PostgREST hit. Pin cardinality estimate per-table in §0 (e.g., `prizma short_links count ≈ 8200 → T2 → embedded fetch via FK`). See memory `feedback_probe_biggest_production_tenant` — probe the biggest production tenant, not just demo.

   - **§5.4e Click-vs-action disambiguation for metrics (added 2026-05-20 from `M4_SHORT_LINKS_DASHBOARD_REDESIGN` P-AUTHOR-5).** Every metric in the SPEC (CTR%, unsubscribe rate, conversion rate, "action count") MUST disambiguate "click event" vs "business state" at SPEC-author time. SMS-gateway link-preview bots fire ~95% of clicks within the first 6 minutes after send — using click logs as a proxy for customer action measures bot behavior, not customer behavior. Rule: source from a business-state column (`crm_leads.unsubscribed_at`, `crm_event_attendees.purchase_amount`, `crm_leads.status`) whenever one exists; only fall back to event logs when no state column exists, and label the metric "raw" with a bot-decontamination caveat. See memory `feedback_clicks_are_not_actions`.

   - **§5.4f Destructive-DML snapshot mandate (added 2026-05-20 from `M4_SMS_RATE_LIMIT_HOTFIX_2026_05_20` P-AUTHOR-2).** When the SPEC declares any destructive DML (`UPDATE`, `DELETE`, mass status flip) under Iron Rule 32, the Foreman MUST also declare the matched-pair snapshot path in §4 (e.g., `_archive/<spec_slug>/pre-update-snapshot.json` with the exact SELECT that captures the rows + their pre-change column values). The Executor reads the SPEC and writes the snapshot BEFORE running the destructive UPDATE/DELETE. This makes post-incident rollback a single `UPDATE ... FROM snapshot` instead of a multi-hour data-archaeology session.
