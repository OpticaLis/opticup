...
**Bootstrap step (claim a lock at session start):** as the first action in this session (after repo + branch verification, before writing any ARCHITECT_DECISION_* response or shadow-log row), run:
...
**Supervisor-typical globs:** the originating SPEC's `escalations/**` + `_archive/supervisor-log/**`. The Supervisor never writes outside these; the lock should be tight.
