...
| 5 | Customers | 🟢 **Phase A+B schema CLOSED 2026-05-22** (Full-Auto Pipeline overnight chain Half 1) | opticup | Person-entity foundation. 8 new tables (households, health_funds, tenant_languages, customer_notes, customer_documents, tenant_settings, tenant_number_counters + customers extended additively) + 4 enums + 7 views (M5 customer-data) + 5 RPCs + 1 shared helper (`allocate_tenant_number`) + 2 deferred trigger functions. tenant_code added to tenants (prizma='01', demo='02'). tenant_location.deactivated_at added. 9/9 functional smoke PASS on demo + 0 Prizma row writes. Iron Rules 1, 11, 14, 15, 18, 19, 22, 32 conformed. Sealed under `modules/Module 5 - Customers/docs/specs/M5_SCHEMA/`. Remaining phases: C (OpticPlus migration), D (customer card UI), E (customer list UI). |
...
| 7–22 | Future modules | ⬜ Not started | — | Orders, payments, lab/KDS, lenses, branches, WhatsApp, reports, supplier portal, content hub, B2B network, AI support, WooCommerce sync, POS. **M7 unblocked** by M5+M6 schema closure (orders FKs `customer_id` + `prescription_id` now stable). |
