...
| RULE18-COMMENT-FALSE-POSITIVE | rule-18-unique-tenant.mjs UNIQUE_RE matches `(NNN)` patterns inside `--` line comments and `/* */` block comments — 1 occurrence currently (line 767 of M1 db-schema.sql, worked around with 2-char comment edit) | M1A_DEBT_SWEEP FINDINGS-03 (2026-05-15) | OPEN. Proposed fix: strip line + block comments from `content` before applying `UNIQUE_RE` in the hook. Effort ~15 min + self-test. Bundle into a future verify-hooks maintenance Pipeline; recommended before Phase 1B starts touching shared SQL docs. |
