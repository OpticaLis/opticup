...
| 1.5 | Shared Components | ✅ Complete + 🟢 **cat-sidebar component shipped 2026-05-17** | opticup | Cross-module infrastructure: shared JS/CSS components (Modal, Toast, TableBuilder, PIN modal, **cat-sidebar**), activity_log, auth/permissions refactor, plan helpers, design tokens. 14 tables. **`M1_5_CAT_SIDEBAR_COMPONENT` 🟢 (2026-05-17 morning, Full Auto Pipeline ~1.5h):** sidebar extracted from inventory.html to reusable `shared/js/cat-sidebar.js` (192 lines, first ES Module in shared/js — divergence from existing IIFE+window M1.5 pattern, documented) + `shared/css/cat-sidebar.css` (162 lines: grid host + sidebar visual + responsive @media). Replaced selector-specific overlap hotfix with `.cat-sidebar-host { display: grid; grid-template-columns: 1fr 240px; }` structural rule — fixes Daniel's contactNav + accessoryNav overlap bug uniformly for all current + future nav strips. Future modules (M5/M7/M9/...) consume via `import { initCatSidebar } from '/shared/js/cat-sidebar.js'`. 8 commits, 0 escalations, 0 DB ops, 0 main touches. 1 cosmetic R-FINDING-1 (3 icon glyph drifts) flagged for Daniel decision in Hebrew summary. |
