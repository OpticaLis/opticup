...
|---|--------|------|--------|------|---------------|
| 1 | Inventory ERP | ✅ Complete (frames era) + 🟢 Lens department production-complete 2026-05-16 | opticup | Full ERP: inventory, purchasing, receipts, supplier debt, returns, shipments, AI-OCR, alerts, stock counts, Access sync. 12 phases (0 → 5.9 + QA). 36 tables. **Lens expansion 2026-05-12 → 2026-05-16:** 10 SPECs closed — schema (18+3 tables) + Phase 1A platform admin + Phase 1B Foundation (read screens) + Procurement (PO + goods-receipt) + Gap Closure (PO state updates + variant-less manual lines + ➖ adjustment RPC) + Phase 2 Completion (RPC harmonization + 31 FK indexes + main-menu wiring with `lens-nav-strip.js` widget + "מחלקת עדשות" card on index.html). **Department now production-complete: staff accesses 7 screens from main menu with permission gating; Prizma untouched throughout.** M7 unblocked. Contact-lenses + accessories phases NO LONGER GATED — D-M1-09 reframed from code-extraction to UX-consistency mandate via design-system SPECs. |
