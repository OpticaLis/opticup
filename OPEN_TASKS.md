...
**Last updated:** 2026-05-12 (**MIGRATION_3_CRM CLOSED 🟢** via Full-Auto Pipeline in ONE chat — third of 4 production-page migrations to Hybrid+Navy. CRM was already on a modern Slate palette, so this is an **accent insertion** (Navy `#1e3a8a` on primary actions, focus rings, view-toggle, sidebar active marker, theme-dot, loading spinner), NOT a full re-skin. Shape differs from Migration #1+#2: CRM relies on inline Tailwind utility classes (`indigo-*`) in `crm.html` instead of CSS rules. Resolution: swap inline classes to Tailwind arbitrary values (`bg-[#1e3a8a]`, `focus:ring-[#1e3a8a]`) — first-class v3 JIT feature, no `!important` wars with Tailwind's `important:true` config. **New pattern validated** for inline-utility-class re-skins (Migration #4 may face same shape on compiled Tailwind). Files: `crm.html` (8 token sites, 6 indigo-* lines + theme-dot inline style — line count unchanged at 419), `css/crm.css` (3 token-value swaps + sidebar Navy box-shadow marker), `css/crm-components.css` (additive `.crm-badge-primary` Navy variant). `crm-screens.css` + `crm-visual.css` untouched (post-B8 stubs, no accent-bearing rules). `shared/css/variables.css` untouched (Navy tokens added by Migration #1, idempotent). Sidebar dark theme preserved. **Verification:** 18/18 SPEC §3 criteria GREEN; 75 `<script>` + 12 `<link>` preserved; 0 `indigo-*` remaining (was 6); 0 legacy purple anywhere; smoke 7/7 PASS; integrity exit 0; page-scope confined (inventory.html has 0 Navy). Localhost-Tester GREEN on HTTP + smoke + page-scope confinement (v1 boundary). Pre-commit tag `pre-migration-crm` at `0dfa6b9`. 2 commits (C1 `1176a89` migration + C2 retrospective). 4 skill improvements applied: opticup-strategic Author #1 (no fractional section numbers — `## 6.5.` blocked C1 hook for ~20s, swapped to plain `## Destructive Operations`), Author #2 (codify pre-existing-untracked-files leave-alone in §0, 4th SPEC in a row to make this decision). opticup-executor #1 (Tailwind utility-class swap pattern — arbitrary values over !important overrides), #2 (pre-execution heading-regex check at SPEC-load time, not commit time). 3 findings opened: F1 (`crm-screens.css` + `crm-visual.css` are post-B8 stubs — future `M1_5_CRM_CSS_STUB_CLEANUP` SPEC), F2 (orphan Tailwind config color tokens → TECH_DEBT), F3 (sidebar marker uses physical `-3px` for RTL — LTR concern, → TECH_DEBT). 3 of 4 production migrations now closed on develop; next: Migration #4 (Storefront Studio).)
...
| 2 | **🔄 Migration of existing screens to Hybrid design system** — 4 in-production screens with Hybrid mockups. **Migration #1 (Suppliers Debt) ✅ CLOSED 2026-05-11** on develop, tag `pre-migration-suppliers-debt`. **Migration #2 (Settings + Permissions) ✅ CLOSED 2026-05-11** on develop, tags `pre-migration-settings` + `pre-migration-employees` (per-page revert). **Settings + Permissions Consolidation ✅ CLOSED 2026-05-12** — `SETTINGS_PERMISSIONS_CONSOLIDATION` SPEC ran the deferred structural change ahead of schedule (single Full-Auto Pipeline chat, tag `pre-consolidation-settings-permissions`). `employees.html` archived; `settings.html` now tabbed. **Migration #3 (CRM) ✅ CLOSED 2026-05-12** on develop, tag `pre-migration-crm` at `0dfa6b9`, commit `1176a89`. Accent insertion (Navy `#1e3a8a` on primary actions, focus rings, view-toggle, sidebar marker) — NOT a full re-skin (CRM was already on modern Slate). Tailwind arbitrary-value swap pattern validated (`bg-[#1e3a8a]` over `!important` overrides) for inline-utility-class re-skins. 75 `<script>` + 12 `<link>` preserved; 0 `indigo-*` remaining; smoke 7/7 PASS; page-scope confined. 3 findings: F1 (future `M1_5_CRM_CSS_STUB_CLEANUP` for `crm-screens.css`/`crm-visual.css` stubs), F2+F3 (TECH_DEBT). Future `M1_5_DEDUPLICATE_SETTINGS_EMPLOYEES_CSS` follow-up SPEC scope: dedup settings/employees byte-identical CSS + remove `css/employees.css` `<link>` from settings.html + bundle with M1_5_CRM_CSS_STUB_CLEANUP. **Next-up: Migration #4 (Storefront Studio)** — Architect writes brief → Full Auto. After Migration #4 lands and passes QA, ONE batch merge to main. Shipments mockup transfers to M9 as a sketch. | Architect writes briefs → Full Auto | 1 day remaining (Storefront Studio) | Second — low-risk migrations to validate the Pipeline on real production code. Pipeline now battle-tested on 4 SPECs (Migration #1 + Migration #2 + Consolidation + Migration #3). |
