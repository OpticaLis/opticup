...
**Last updated:** 2026-05-13 evening (`BROADCAST_EVENT_LINK_SUPPORT` 🟢 CLOSED via Full-Auto Pipeline — CRM Broadcast Wizard now carries `event_id` end-to-end through `crm_message_queue.event_id` so `send-message` EF substitutes `%registration_url%` per recipient. **Event #24 rescue dispatch READY — ready as soon as the wizard fix lands** (it just did). Daniel can re-open the Broadcast Wizard on `app.opticalis.co.il/crm.html#messaging`, pick Event #24 from the new step-3 "Linked event" dropdown, re-send to the 1,187 eligible Prizma leads — same body as 2026-05-13 morning attempt; the per-lead URL will now substitute correctly. **Active task before this hotfix: M1 EXPANSION — Lens Inventory schema design (in progress)**. Returning to that after Daniel re-sends Event #24.
