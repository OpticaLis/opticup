...
**Last updated:** 2026-05-11 latest session (**DEMO_WHITELIST_UPDATE CLOSED 🟡** via Full-Auto Pipeline in ONE chat. Diagnostic-first: demo's `test_mode_sms_allowlist` ALREADY contains exactly Daniel's 3 phones in E.164 (mandated by C-001) — NO SMS UPDATE applied; demo SMS envelope ready for manual test cycle. Email allowlist mechanism does NOT exist anywhere (no column, no `ui_config` jsonb key, no `send-message` EF gate) — wrote `ESCALATION.md` with 3 options + Foreman recommendation Option 2 (jsonb in `ui_config`, minimal disruption). Zero DB writes, zero code changes, Prizma row untouched (`updated_at` identical pre/post). New Active row #1.5 for Architect's Option 1/2/3 decision. 4 skill improvements harvested for opticup-strategic + opticup-executor (SPEC envelope narrowing; ESCALATION.md as first-class artifact; no-op-verify checklist; diagnostic-first pre-authoring pattern in Full-Auto mode).)
...
|---|---|---|---|---|
| 1 | **📧 Email allowlist mechanism — Architect decision needed (3 options)** — `DEMO_WHITELIST_UPDATE` SPEC surfaced that `tenants` has NO `test_mode_email_allowlist` column, NO `ui_config` jsonb key for email allowlisting, and `send-message` EF v21 has zero email-side gating. Today demo's email dispatches go to whatever address is in `crm_leads.email` with no envelope. Three options enumerated in `modules/Module 4 - CRM/docs/specs/DEMO_WHITELIST_UPDATE/ESCALATION.md`: (1) add column mirroring SMS pattern (half-day, 3 commits); (2) **RECOMMENDED** — add `ui_config.test_mode_email_allowlist` jsonb key + 10-line EF helper (same-day, 2 commits, ships TODAY); (3) accept-as-debt + operator discipline (zero now, full SPEC cost later). Daniel's manual test cycle starts soon — gap is bounded (demo has few leads) but non-zero. SMS side already ready (no-op verified). | Architect (Daniel decides) → Full-Auto SPEC | ~half day if Option 2 | BLOCKER for safe email automations during Daniel's manual test cycle. |
| 2 | **🔍 M13 + M9 sketch revision — the 2 outliers** — Batch 3 (M5/M6/M8/M11/M12/M14/M15) closed 2026-05-11 via `M1_5_SKETCH_RESKIN_BATCH_3` — 17 files re-skinned to Hybrid+Navy. Two outliers remain: (a) **M13 Loyalty** uses a Prizma-gold palette with gradients — needs full revision, not just re-skin (gradients + gold contradict SaaS-clean design system); (b) **M9 Lab/KDS** has no sketches at all — sketches-from-scratch with Daniel involvement. Both are separate Batches per Brief §3 of Batch 3. | Architect (Daniel decides) | ~half day | Last sketch work before module builds. M7 already Hybrid+Navy (V7 Variant A). |
| 3 | **🔄 Migration of existing screens to Hybrid design system** — 4 in-production screens with Hybrid mockups. **Migration #1 (Suppliers Debt) ✅ CLOSED 2026-05-11** on develop, tag `pre-migration-suppliers-debt`. **Migration #2 (Settings + Permissions) ✅ CLOSED 2026-05-11** on develop, tags `pre-migration-settings` + `pre-migration-employees` (per-page revert). Daniel decision: the 2 stay as separate pages; the tab-consolidation Hybrid mockup proposed earns a separate SPEC AFTER all 4 visual migrations land (structural change: routing, event handlers, link migration). **Next-up: Migration #3 (CRM)** — Architect writes brief → Full Auto. Then Migration #4 (Storefront Studio). After all 4 land on develop and pass QA, ONE batch merge to main + the deferred consolidation SPEC. Shipments mockup transfers to M9 as a sketch (not a standalone migration — happens with M9 build). | Architect writes briefs → Full Auto | 2 days remaining (1/screen) | Second — low-risk migrations to validate the Pipeline on real production code. Pipeline now battle-tested on 2 SPECs. |
| 4 | **🚀 Build 10 new modules on Hybrid system** — M5, M6, M7, M8, M9 (includes shipments+boxes from absorbed M1.shipments), M11, M12, M13, M14, M15. Each gets a Foreman-authored Brief if needed (or uses existing Brief), then Full Auto Pipeline. Order: M5/M6/M7 foundation first, then M8/M9/M11/M12/M13 in parallel across split repos. Permissions sub-tab arrives with M5+M2 admin layer. M7 uses the locked V7 (Variant A) sketch. | Architect briefs → Full Auto chain | 6-8 weeks | Critical path to LIVE. |
| 5 | **📦 Module Repo Split** — Split `opticalis/opticup` into per-module repos + `opticup-shared` (design tokens + shared.js + components). Enables 3-4 parallel Full Auto pipelines without git conflicts. | Architect + Executor | 1-2 days | After migrations — once Pipeline is proven on monorepo, split makes parallel modules safe. |
...
| 7 | **M13 Brief amendment** — add basic-free membership type (auto-created on first compensation/Referral, no-fee, no-bonus, but receives credits). Edit `modules/Module 13 - Loyalty Club/architecture-brief/M13_LOYALTY_BRIEF.md` D5 + add new entity slot. Surfaced 2026-05-10 during M9 D24. | Architect (Daniel) | ~30 min | Required before M9 build SPECs (M9 uses M13 entity). Can run anytime in parallel. |
