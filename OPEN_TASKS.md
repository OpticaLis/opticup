...
**Last updated:** 2026-05-11 latest session (**DEMO_EMAIL_ALLOWLIST_INFRA CLOSED 🟢** via Full-Auto Pipeline in ONE chat. Architect's Option 2 from predecessor escalation applied: `ui_config.test_mode_email_allowlist` (jsonb path) over a new column or separate table. `send-message` EF v21→v22: `phoneAllowed` + new `emailAllowed` extracted to co-located `allowlists.ts` (Rule 12 forced the extraction; SMS body byte-identical). Email gate mirrors SMS contract (empty/missing = send to all; non-empty = filter; fail-CLOSED on lookup error or malformed jsonb). Demo gets 3 emails (`danylis92@gmail.com`, `daniel@prizma-optic.co.il`, `alkimovich94@gmail.com`) via single-row `jsonb_set` UPDATE; demo `ui_config` keys grew 12→13, all existing keys preserved. Prizma untouched — `ui_config ? 'test_mode_email_allowlist'` returns false, `updated_at` identical pre/post. Demo's `updated_at` did NOT change post-UPDATE (confirms F3 from predecessor — no `updated_at` trigger on `tenants`; pre-existing TECH_DEBT). Demo now ready for the full manual test cycle (SMS + email envelopes both locked to Daniel's inboxes). Active row #1 closes.)
