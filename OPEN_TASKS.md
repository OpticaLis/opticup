...
**Last updated:** 2026-05-12 (**PRIZMA_CRM_BUGFIX_BACKPORT CLOSED 🟢** via Full-Auto Pipeline in ONE chat — backport of the 2026-05-11 demo E2E audit data fix to Prizma production. 2 single-row UPDATEs on `crm_automation_rules` (Prizma's `d2585fc4` + `c25feaf7` — the analogs of demo's `a06be5d8` + `ee0a6f24`). Each rule's `action_config` rewritten: `recipient_type` `cross_event_active_waitlist` → `leads_by_status`, added `recipient_status_filter=['waitlist']`, removed `post_action_attendee_upsert={status:'invited'}` key. **Path A** auto-decided: Prizma's pre-fix shape matched demo's pre-fix SNAPSHOT byte-for-byte structurally; new md5s are byte-identical to demo's post-fix md5s. Prizma's 14 non-target rules aggregate md5 unchanged. Demo's 2 rules unchanged. **EF dry-run** (`automation-engine` v8 `mode='evaluate'`) on Prizma + test event for both triggers: 0 outbound, 0 attendee inserts, 0 queue writes (verified: 0 `crm_message_log` rows tied to the 4 dry-run `run_id`s). Pre-write annotated git tag `pre-backport-prizma-event-invite-fix` on `bccbc1a`. Pre-merge artifacts: `READY-FOR-MAIN-MERGE.md` (PR title/body), `ROLLBACK_SQL.md` (verbatim pre-state SQL), `ARCHITECT_REVIEW_CHECKPOINT.md` (🟢 verdict). **Main-merge is Daniel-only** via GitHub PR. 4 skill improvements harvested (2 author + 2 executor) — to be applied next session. 2 findings opened: `M4-DEBT-CRM-AUTO-RULES-UPDATED-AT` (no `updated_at` column on rules table) + `M4-DEBT-EVENT-REG-OPEN-AUDIENCE-AUDIT` (separate rule resolves to 1999 plan_items on `registration_open` — audit-worthy outbound volume question).)
...
| 2 | **🔄 Migration of existing screens to Hybrid design system** — 4 in-production screens with Hybrid mockups. **Migration #1 (Suppliers Debt) ✅ CLOSED 2026-05-11** on develop, tag `pre-migration-suppliers-debt`. **Migration #2 (Settings + Permissions) ✅ CLOSED 2026-05-11** on develop, tags `pre-migration-settings` + `pre-migration-employees` (per-page revert). **Settings + Permissions Consolidation ✅ CLOSED 2026-05-12** — `SETTINGS_PERMISSIONS_CONSOLIDATION` SPEC ran the deferred structural change ahead of schedule (single Full-Auto Pipeline chat, tag `pre-consolidation-settings-permissions`). `employees.html` archived; `settings.html` now tabbed (כללי + הרשאות). Existing `showTab()` reused (Iron Rule 21). 0 LIVE refs to old path. Future `M1_5_DEDUPLICATE_SETTINGS_EMPLOYEES_CSS` follow-up SPEC scope expanded: also remove `css/employees.css` `<link>` from settings.html when it runs. **Next-up: Migration #3 (CRM)** — Architect writes brief → Full Auto. Then Migration #4 (Storefront Studio). After all 4 land on develop and pass QA, ONE batch merge to main. Shipments mockup transfers to M9 as a sketch (not a standalone migration — happens with M9 build). | Architect writes briefs → Full Auto | 2 days remaining (1/screen) | Second — low-risk migrations to validate the Pipeline on real production code. Pipeline now battle-tested on 3 SPECs (Migration #1 + Migration #2 + Consolidation). |
