...
**Last updated:** 2026-05-09 (after M3_SITEMAP_CONSOLIDATION close — REC-SITE-011 closed via canonical-www + branches-included sitemap)
...
| REC-SITE-018 | LOW | `getBaseUrl(tenant, request)` returns apex (no www) for tenants whose `storefront_config.custom_domain` is set without `www`. Affects OG meta tags + Schema.org JSON-LD canonical URLs across the site (sitemap was switched off this code path in M3_SITEMAP_CONSOLIDATION but other consumers remain). Either UPDATE `storefront_config.custom_domain` to `www.prizma-optic.co.il` (1 SQL UPDATE) or refactor `getBaseUrl`. Source: M3_SITEMAP_CONSOLIDATION/FINDINGS.md M3-DATA-04. | Trivial (1 SQL UPDATE) or 30-min refactor. |
