# פרומפט הפעלה — FOREMAN_REVIEW של M4_CAMPAIGNS_SCREEN

> העתק את כל מה שמתחת לקו ל-Claude Code. **באותו הסשן.**

---

צור את הקובץ `modules/Module 4 - CRM/docs/specs/M4_CAMPAIGNS_SCREEN/FOREMAN_REVIEW.md` עם התוכן הבא:
...
```
git add "modules/Module 4 - CRM/docs/specs/M4_CAMPAIGNS_SCREEN/FOREMAN_REVIEW.md"
git commit -m "docs(spec): add FOREMAN_REVIEW for M4_CAMPAIGNS_SCREEN (verdict: 🟡 CLOSED W/ FOLLOWUPS)"
git push origin develop
```

ואז מחק את הקובץ הזמני מ-outputs:
```
rm outputs/M4_CAMPAIGNS_SCREEN_SPEC_DRAFT.md
git add outputs/
