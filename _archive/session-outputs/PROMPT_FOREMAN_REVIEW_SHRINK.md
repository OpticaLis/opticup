# פרומפט הפעלה — FOREMAN_REVIEW של M1_RECEIPT_PO_COMPARE_SHRINK

> העתק את כל מה שמתחת לקו ל-Claude Code. **באותו הסשן.**

---

צור את הקובץ `modules/Module 1 - Inventory/docs/specs/M1_RECEIPT_PO_COMPARE_SHRINK/FOREMAN_REVIEW.md` עם התוכן הבא:
...
```
git add "modules/Module 1 - Inventory/docs/specs/M1_RECEIPT_PO_COMPARE_SHRINK/FOREMAN_REVIEW.md"
git commit -m "docs(spec): add FOREMAN_REVIEW for M1_RECEIPT_PO_COMPARE_SHRINK (verdict: 🟢 CLOSED)"
git push origin develop
```

לאחר מכן — מחק את הקובץ הזמני מ-outputs:
```
rm outputs/M1_RECEIPT_PO_COMPARE_SHRINK_SPEC_DRAFT.md
git add outputs/
git commit -m "chore(outputs): remove stale SPEC draft after canonical commit (Rule 21)"
git push origin develop
```

דווח: 2 hashes חדשים + git status נקי.
