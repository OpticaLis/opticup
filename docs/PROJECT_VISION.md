# Optic Up — Project Vision

> **Source:** Originally extracted from the Miro board "Optic Up Project New"
> (https://miro.com/app/board/uXjVIMS5jkU=) on 2026-03-08 as part of Module 1
> planning. Promoted to project-level foundation by Phase 3C of Module 3.1
> on 2026-04-11.
>
> **Status:** Historical vision document. Some modules listed here are
> complete (Modules 1, 1.5, 2, partial 3), others are deferred for
> future development. This file is the authoritative reference for the
> full 28-module Optic Up product ambition.
>
> **Consolidated from:**
> - `modules/Module 1 - Inventory Management/docs/SPEC.md` (v1.0, full spec)
> - `modules/Module 1 - Inventory Management/docs/OPTIC_UP_PROJECT_GUIDE_v1.1.md` (v1.1, added Module Contracts, DB Status, Decision Log, Goods Receipt flow)
>
> **Board stats:** 8,262 items total (923 text, 6,183 shapes, 444 stencils/wireframes, 350 images, 290 connectors, 71 groups). Approximately 90–95% of the board content was extracted programmatically and visually verified.
>
> **Note:** The original v1.1 GUIDE included a "Part 0" Hebrew section on collaboration workflow (the "Daniel writes prompts, Claude Code executes" model). That guidance is obsolete — superseded by `CLAUDE.md` §9 (Bounded Autonomy). It is intentionally not reproduced here.
...
## 5. Module Contracts (Historical Snapshot — March 2026)

> Originally documented in OPTIC_UP_PROJECT_GUIDE_v1.1 §5. Inter-module contracts — every new module that needs inventory access must call these functions, never reach into inventory tables directly. **This snapshot reflects Module 1 v1.0 state. The current authoritative function registry is `docs/GLOBAL_MAP.md`.**
...
## 6. Database Status (Historical Snapshot — March 2026)

> Originally documented in OPTIC_UP_PROJECT_GUIDE_v1.1 §6. **This snapshot reflects March 2026. The current authoritative schema is `docs/GLOBAL_SCHEMA.sql` and `docs/DB_TABLES_REFERENCE.md`.**
...
**Extensions enabled (March 2026):**
- `uuid-ossp` — UUID generation
- `pg_trgm` — Fuzzy text search (GIN indexes on model, color)

**RLS (March 2026):** Enabled on all 8 built tables. Open access (`USING (true)`) at the time. Has since been tightened with multi-tenant policies under SaaS Rules 14–15.

---

## 7. Module Development Status (Historical Snapshot — March 2026)

> Originally documented in OPTIC_UP_PROJECT_GUIDE_v1.1 §7. **This snapshot is frozen. Current per-module status lives in `modules/Module X/docs/SESSION_CONTEXT.md` and `modules/Module X/ROADMAP.md`.**
...
## 8. Architectural Decision Log (Historical Snapshot — March 2026)

> Originally documented in OPTIC_UP_PROJECT_GUIDE_v1.1 §8. Key architectural and business decisions made during the Module 1 development period.
...
### 9.9 Goods Receipt Flow (added in v1.1, March 2026)
...
18. **Goods Receipt** — 2-step flow (list → form), added v1.1
19. **System Log** — Admin-only, filterable audit trail
...
13. ✅ **Barcode Format:** RESOLVED — BBDDDDD format (2-digit branch + 5-digit sequence). See `docs/CONVENTIONS.md` and Iron Rule 4.

---

## 12. Priority Order (Based on Board Analysis)
...
4. **Frames Inventory** ✅ COMPLETE (v1.0, March 2026) — Primary product management with barcode system
...
6. Inventory quantity changes only through Add/Remove prompts, not manual editing (PIN required)
7. Barcodes: BBDDDDD format (2-digit branch + 5-digit sequence), ascending, unique per system
...
11. Clients not acquired through tracked campaigns but making first purchase → count as campaign-acquired

---

## 14. Confidence Assessment

Approximately **90–95%** of the Miro board content was extracted and read through Miro's programmatic API, supplemented by visual verification of key wireframe areas. The board contains 8,262 items across a massive canvas spanning nearly 1 million coordinate units wide. 956 meaningful text items (114,628 characters) were successfully extracted, plus hundreds of shape labels. Some content (~5–10%) was blocked by content filters during extraction but was recovered through alternative reading offsets. The main areas fully captured include: all specification documents (Lead Management, Finance, Queue System, Contact Lenses, Inventory, Supplier Portal, Reports), all WhatsApp message templates (Hebrew, Russian, English), all wireframe labels and UI element descriptions, user roles and permissions, and business rules. The wireframe visual layouts were verified through screenshots but exact pixel-level arrangements are best reviewed directly on the Miro board. Some visual details in the 444 stencil items and 350 images (screenshots, mockups) could not be read as text — these represent UI wireframe templates that complement the textual specifications captured above.

---

## Provenance & Version History

- **2026-03-08** — v1.0 of `SPEC.md` extracted from Miro board "Optic Up Project New" as part of Module 1 planning. 28 modules documented in full.
- **2026-03-08** — v1.1 of `OPTIC_UP_PROJECT_GUIDE_v1.1.md` published. Added Module Contracts (§5), DB Status (§6), Module Development Status (§7), Decision Log (§8), and Goods Receipt Flow. Marked Frames Inventory as complete.
- **2026-04-11** — Phase 3C of Module 3.1: both files consolidated into this single `docs/PROJECT_VISION.md`, promoted to project-level. Original files removed; backups stored at `modules/Module 3.1 - Project Reconstruction/backups/M3.1-3C_2026-04-11/`. The obsolete v1.1 "Part 0" workflow guidance was intentionally not carried forward (superseded by `CLAUDE.md` §9 Bounded Autonomy).
