...
async function loadPOListForEntry() {
  ...
  try {
    const recs = await fetchAll(T.PO, [['status','in',['ordered','partial']]]);
...
async function loadPOForEntry(poId, poNum, poSupplier) {
  ...
  try {
    const items = await fetchAll(T.ITEMS, [['purchase_order_id','eq',poId]]);
...
async function submitFromPO() {
  ...
  try {
    if (arrived.length) {
      ...
      const created = await batchCreate(T.INV, invRecords);
      // Log each item from PO
      for (const item of created) {
        writeLog('entry_po', item.id, {
          barcode:    item.fields['ברקוד'],
          brand:      item.fields['חברה / מותג'],
          model:      item.fields['דגם'],
          qty_before: 0,
          qty_after:  item.fields['כמות'] || 1,
          source_ref: selectedPOForEntry.num
        });
      }
    }
    ...
    await batchUpdate(T.PO, [{ id: selectedPOForEntry.id, fields: { 'סטטוס הזמנה': newStatus } }]);
    ...
    setAlert('po-items-alerts', 'שגיאה: ' + msg, 'e');
  }
  hideLoading();
}
