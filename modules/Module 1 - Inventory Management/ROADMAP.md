...
- **מבנה הסכמה (3 שכבות):** Global Catalog (lens_brand → lens_design → lens_variant) → Commercial (supplier_catalog_offering, pricing_overlay) → Retailer (tenant_active_offerings, tenant_lens_stock, tenant_location) → Operations (stock_lot, stock_movement, stock_transfer, purchase_receipt, purchase_receipt_line) + Governance (supplier_permissions, change_approval_log, vat_rates)
- **17 טבלאות חדשות, 1 עמודה חדשה על tenants** (base_currency_code), 9 RPCs אטומיים, 1 טריגר Postgres (K3 — m9_lens_received_for_sale_order), 1 View (K5 — v_suppliers_for_m9)
- **מסך:** Platform Catalog Admin בלבד (mockup #5) — צוות Optic Up בלבד, מוסתר מהאופטיק
- **Edge Function:** lens-catalog-import — ייבוא קטלוג מותג שלם מ-xlsx (rules-based; LLM agent דחוי ל-Phase 2+)
- **17 T-constants חדשים ב-js/shared.js** + עדכון FIELD_MAP לכל עמודה חדשה (Iron Rule 5)
- **חוזים ל-M9:** purchase_receipt.shipping_box_id (additive, NULL, ללא FK clause עד ש-M9 בונה את shipping_boxes); v_suppliers_for_m9 View
- **לא בתוך הסקופ:** 6 מסכי לקוח (Lens-1B), הרחבת `modules/goods-receipts/` עם product_category dispatcher (Lens-1B), מחיקת legacy shipments (M9 SPEC), מסכי עדשות-מגע/אביזרים (פאזות M1 עתידיות)
- **Open Question Resolutions** (Brief §7):
  - Q1 (טבלאות naming) — אופציה (c) divergence: purchase_receipt חדש; goods_receipts הקיים נשאר לפריימים. Code reuse בשכבת ה-UI דרך product_category dispatcher (Lens-1B).
  - Q2 (Variant ID) — UUID PK + display_id TEXT UNIQUE generated by next_lens_variant_display_id() RPC, פורמט LV-NNNNNN.
...
- **6 מסכים:**
  1. Lens Inventory Management — מטריצת SPH×CYL לכל variant פעיל, צוות חנות יומיומי
  2. Active Designs Selection — מנהל סניף בוחר אילו lens series האופטיק נושא
  3. Catalog & Pricing — 3 עמודות (catalog/discount%/final), inline edit, bulk required (D-M1-04)
  4. Purchase Order — per-supplier, מחולק לפי source (stock-shortage / custom-per-customer / manual), manual export PDF/Excel
  6. Active POs List — display-only across all suppliers, status pipeline (Draft → Sent → Partial → Full)
  7. Goods Receipt — מבוסס על תבנית הקבלה הקיימת לפריימים (`modules/goods-receipts/`), משתמש ב-RPC m1_create_receipt_from_box מ-Lens-1A
- **Open Question #1 — code-layer extension:** הרחבת `modules/goods-receipts/*.js` עם product_category dispatcher ('frames' → goods_receipts, 'lenses' → purchase_receipt). שמירה על Iron Rule 21 בשכבת הקוד.
- **בנייה לאחר** רישום קטלוג demo ראשוני בעזרת Platform Catalog Admin (כדי לבדוק את הזרימה end-to-end על נתונים אמיתיים)
