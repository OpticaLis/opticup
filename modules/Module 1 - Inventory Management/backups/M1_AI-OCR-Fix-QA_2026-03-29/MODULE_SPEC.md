...
## גרסה Phase 8 + Debt Module Upgrades | מרץ 2026
...
**סטאק טכנולוגי:**
- Frontend: Vanilla JS (no framework), ~140 JS modules + CSS
...
- **Auto-deduct from active prepaid deal** on receipt confirm (autoDeductPrepaid in receipt-debt.js), falls back to alert if insufficient
- **Receipt header redesign**: field order ספק → PO → סוג מסמך → כמות מסמכים → מספר → תאריך
- **Dynamic multi-doc inputs**: set count → N number+amount rows (receipt-doc-numbers.js)
- **PO availability indicator**: green border if open POs exist for supplier
- **AI auto-detect**: supplier match from OCR (receipt-ocr-supplier.js), PO auto-match (receipt-ocr-po.js), doc type learning
- **OCR flow enhancements**: PO choice modal, cached re-scan, comparison highlights (receipt-ocr-flow.js)
...
- **Suppliers table**: aggregated per supplier — open doc count, total debt, overdue amount, next due date, prepaid deal (green=prepaid / red=charged)
- **Filter chips** (debt-supplier-filters.js): type (defined/general), history (with/without receipts), debt (has_debt/overdue)
- Row click → supplier detail panel (slide-in)
- Quick action: create payment for supplier

#### 3.12.2 טאב מסמכים (Documents Tab)
- **Filter bar**: supplier, type, status, date range, month picker toggle, amount range (min/max), overdue checkbox
- **Documents table**: date, type, number, internal number (DOC-NNNN), supplier, amount, paid, balance, status badge
- **Multi-doc expand**: count badge "N 📄" on multi-document entries, expand to sub-row with breakdown
- **Full document editing**: supplier change (PIN), amounts, status transitions, subtotal always editable
...
- **Auto-deduct from prepaid deal**: new invoices auto-deduct from active deal if sufficient balance
...
- **4-step payment wizard**:
  ...
  3. Document allocation (FIFO auto-allocate or manual override, mismatch warning, pre-selected docs highlighted ★)
  4. Confirmation (summary + PIN)
- **Pay from document**: openPaymentForDocument pre-fills wizard for specific doc
- **Multi-select payment**: checkboxes on payable docs + "שלם נבחרים" action bar in supplier detail
...
- **Auto-deduction**: receipt confirmation and manual document creation auto-deduct from active prepaid deal (autoDeductPrepaid / _autoDeductFromPrepaid)
- **Prepaid display**: dashboard shows totalPrepaid (green) / totalUsed (red) per supplier
...
- **Path:** supabase/functions/remove-background/index.ts (123 lines)
- **API:** POST { image_base64, session_token } -> { success, image_base64, format, size }
- **Auth:** SUPABASE_ANON + apikey headers pass gateway; session_token validated against auth_sessions
- **Service:** remove.bg API (REMOVE_BG_API_KEY secret)
- **Client:** Choice dialog (AI vs Canvas), no auto-fallback, separate comparison views

### Storage Bucket: frame-images
- **Type:** Private (signed URLs, 1h expiry)
- **Path format:** frames/{tenant_id}/{inventory_id}/{timestamp}.webp
