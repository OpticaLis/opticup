...
| 5 | inventory-table.js | modules/inventory/inventory-table.js | 300 | Main inventory table: server-side paginated loading, filtering (search/supplier/type/qty), sorting, rendering with inline edit cells (product_type clickable), subrow for bridge+temple_length |
| 5b | inventory-actions.js | modules/inventory/inventory-actions.js | 107 | Action menu (⋯ button): _openInvMenu, _closeInvMenu, _toggleSubrow (subrow toggle), invEditSub (inline edit bridge/temple_length in subrow), event delegation for menu/reduce/image/qty buttons |
| 6 | inventory-edit.js | modules/inventory/inventory-edit.js | 342 | Inline editing (invEdit, invEditSync, invEditProductType), row selection (toggle/selectAll/clear), bulk update, bulk delete with PIN, image preview, saveInventoryChanges with writeLog |
...
| 8 | inventory-entry.js | modules/inventory/inventory-entry.js | 318 | Manual entry forms: addEntryRow (card-based), copyEntryRow, removeEntryRow, getEntryRows, validateEntryRows, submitEntry with barcode retry |
| 9 | inventory-export.js | modules/inventory/inventory-export.js | 227 | Barcode generation (BBDDDDD format with reuse), barcode export to xlsx, full inventory export with styled Excel |
...
| 13 | po-form.js | modules/purchasing/po-form.js | 252 | PO creation wizard: openNewPurchaseOrder (step 1), proceedToPOItems (step 2 bridge), openEditPO, renderPOForm, resolveSupplierName. Brand filter bar (po-brand-filter-bar) |
| 14 | po-items.js | modules/purchasing/po-items.js | 346 | PO item management: renderPOItemsTable (product_type dropdown in main row), cascading brand→model→color/size datalists, addPOItemManual, addPOItemByBarcode, removePOItem, duplicatePOItem, updatePOTotals, _calcPOFinalPrice, _onPOFinalPriceChange, _onPODiscountChange, _onPOCostChange, _updatePOStockCounter, _filterPOByBrand, _updatePOBrandFilter |
...
| 16b | po-view.js | modules/purchasing/po-view.js | 260 | PO view with received qty tracking, receipt-based reason badges, editable sell price+discount+product_type columns (sent/partial POs), _savePOViewPrices (saves product_type too), summary row (lines/units/total), PO event delegation |
| 17 | goods-receipt.js | modules/goods-receipts/goods-receipt.js | 358 | Receipt list: loadReceiptTab, loadPOsForSupplier (PO availability indicator: green/gray border), onReceiptPoSelected, updatePOStatusAfterReceipt, openNewReceipt (resets doc count), _initRcptSupplierSelect |
| 17b | receipt-doc-numbers.js | modules/goods-receipts/receipt-doc-numbers.js | 123 | Receipt document number management (split from receipt-form.js): _rcptExtraNums, _rcptDocAmounts, _addRcptExtraNum, _removeRcptExtraNum, _renderRcptExtraNums, getRcptDocumentNumbers, getRcptDocAmounts, _onDocCountChange (dynamic N doc inputs), _onExtraNumEdit, _onExtraAmtEdit |
| 18 | receipt-form.js | modules/goods-receipts/receipt-form.js | 277 | Receipt form setup: file attachment state (_pendingReceiptFiles), _pickReceiptFile, _initReceiptDropzone, _stageReceiptFile, _renderReceiptFileList, _resetReceiptAttachUI, _removeReceiptFileAt, _removeReceiptFile, openExistingReceipt, toggleReceiptFormInputs, searchReceiptBarcode |
| 18a | receipt-form-items.js | modules/goods-receipts/receipt-form-items.js | 344 | Receipt item row management: addReceiptItemRow (with .rcpt-line-total cell), getReceiptItems (includes note), _toggleRcptNote, _onReceiptStatusChange, updateReceiptItemsStats (now writes per-row line-total + calls _updateRcptInvoiceCompare), addNewReceiptRow, generateReceiptBarcodes |
| 18a2 | receipt-form-validate.js | modules/goods-receipts/receipt-form-validate.js | 120 | Receipt form UI validation (sort lock + invoice-total compare): _rcptSortLocked state, gated column-sort handler, toggleRcptSortLock, _initRcptSortLockUI, _updateRcptInvoiceCompare (called from updateReceiptItemsStats), _rcptInvoiceTotalDelta (used by confirmReceipt gate), _initRcptInvoiceCompareListener |
...
| 19 | receipt-actions.js | modules/goods-receipts/receipt-actions.js | 197 | Receipt save/cancel: saveReceiptDraft (persists note+document_numbers), saveReceiptDraftInternal (same), cancelReceipt, backToReceiptList |
| 19b | receipt-confirm-items.js | modules/goods-receipts/receipt-confirm-items.js | 236 | Item-level inventory processing: confirmReceiptCore (passes receipt_type + allDocNumbers + docAmounts to createDocumentFromReceipt), createNewInventoryFromReceiptItem (uses item qty + product_type) |
| 19b2 | receipt-confirm.js | modules/goods-receipts/receipt-confirm.js | 272 | Confirm flow orchestration: confirmReceipt (validates new items have barcode), _confirmReceiptWithDecisions, confirmReceiptById, _receiptPinVerify, _showMatchConfirmDialog |
| 19c | receipt-debt.js | modules/goods-receipts/receipt-debt.js | 202 | Auto-create supplier_documents on receipt confirmation: createDocumentFromReceipt(receiptId, supplierId, receiptItems, documentNumber, documentType, allDocNumbers, docAmounts). Uses receipt form doc type (fallback supplier default → delivery_note). Sets document_numbers[] and document_amounts[] for multi-doc support. Excludes returned+not_received from subtotal. Always creates doc even when subtotal=0 (sets missing_price=true). Auto-deducts from prepaid deal via autoDeductPrepaid(); falls back to alertPrepaidNewDocument if insufficient balance |
| 19d0 | receipt-ocr-supplier.js | modules/goods-receipts/receipt-ocr-supplier.js | 113 | AI supplier auto-detection: OcrSupplierMatch.matchSupplier (alias exact → name exact → fuzzy), OcrSupplierMatch.learnSupplierAlias (saves OCR name as alias on supplier_ocr_templates) |
| 19d1 | receipt-ocr-po.js | modules/goods-receipts/receipt-ocr-po.js | 167 | AI PO auto-matching: OcrPOMatch.findBestPO (scores POs by item count + amount + item match), OcrPOMatch.compareItems (match/qty_mismatch/price_mismatch/not_in_po/missing_from_ocr), _rcptOcrShowPOHint, _applyOcrHighlights (applies CSS classes + tooltips to item rows) |
| 19d2 | receipt-ocr-flow.js | modules/goods-receipts/receipt-ocr-flow.js | 154 | OCR flow helpers: _rcptOcrShowCompareBtn, _rcptOcrHideCompareBtn, _rcptOcrRunComparison (triggers PO comparison from OCR), _rcptOcrCachedRescan (re-scan without re-upload), _rcptOcrShowPOChoiceModal (modal to choose PO when multiple open), _rcptOcrChoosePO, _rcptOcrSkipPO, _rcptOcrLearnDocNum |
| 19d | receipt-ocr.js | modules/goods-receipts/receipt-ocr.js | 352 | OCR integration: initReceiptOCR, _rcptOcrScan, _applyOCRToReceipt (auto-fill supplier/PO/type/number/date with confidence dots), _rcptOcrShowSupplierHint, _rcptOcrFC/_rcptOcrAddConfDot, _rcptOcrSuggestPO, _rcptOcrShowBanner, _rcptOcrPreviewDoc, _rcptOcrUpdateTemplate. receipt-confirmed listener: snapshot OCR before clear, learn supplier alias + doc type correction + ai_has_po_pattern |
| 19e | receipt-ocr-review.js | modules/goods-receipts/receipt-ocr-review.js | 401 | Item matching + review UI: _rcptOcrParseDescription (rewritten: model before size, prefix aliases, multi-word brands, normalized matching), _rcptOcrMatchItem, _rcptOcrClassifyItems, _rcptOcrShowReview, _rcptOcrApplyToForm, _rcptOcrBuildItemCorrections, _rcptOcrSaveItemLearning |
| 19e1 | receipt-ocr-learn.js | modules/goods-receipts/receipt-ocr-learn.js | 120 | OCR learning: _getSupplierLearningStage (fetches from supplier_ocr_templates), _rcptOcrShowStageIndicator (🔴/🟡/🟢 banner), _rcptOcrUpdateTemplate (field counting + RPC), receipt-confirmed event listener |
| 19e2 | receipt-ocr-confirm-learn.js | modules/goods-receipts/receipt-ocr-confirm-learn.js | 159 | Confirm & learn: _rcptOcrConfirmAndLearn (captures items before confirm, matches OCR→table), _matchOcrToTableItems (score-based: model+price+qty), _saveItemAliases (to extraction_hints), _rcptOcrToggleLearnBtn |
| 19e3 | receipt-list.js | modules/goods-receipts/receipt-list.js | 96 | Receipt list: loadReceiptTab (split from goods-receipt.js, summary cards, table with multi-doc "+N" badges) |
| 19f | receipt-po-compare.js | modules/goods-receipts/receipt-po-compare.js | 350 | PO comparison report: _poCompBuildReport (aggregates multi-row receipt items by key for qty comparison; handles receipt_status: not_received→missing, return→returnMarked), _poCompShowReport (7-section modal: matched/shortage/priceGap/notInPo/returnMarked/missing + reorder), _poCompCollectDecisions (auto-sets returned/not_received for marked items), _poCompApplyDecisions (creates supplier_return for returned, logs not_received), _poCompLearnPricePattern (VAT detection), _poCompCreateReorderPO |
| 20 | receipt-excel.js | modules/goods-receipts/receipt-excel.js | 257 | Receipt Excel: handleReceiptExcel (import items), exportReceiptExcel, exportReceiptToAccess, exportReceiptBarcodes (repeats same barcode N times by qty for label printing), receipt list event delegation |
| 21 | audit-log.js | modules/audit/audit-log.js | 184 | Soft delete flow (deleteInvRow, confirmSoftDelete), recycle bin (openRecycleBin, restoreItem, permanentDelete with double PIN + cascade image delete) |
...
| 22b | entry-history.js | modules/audit/entry-history.js | 218 | Entry history accordion: openEntryHistory, loadEntryHistory, renderEntryHistory, toggleHistGroup, exportDateGroupBarcodes (repeats barcode by qty for labels), ENTRY_ACTIONS constant |
...
| 31c | stock-count-scan.js | modules/stock-count/stock-count-scan.js | 267 | Scan logic: barcode normalization (5 strategies), manual search, row click, handleScan dispatch, qty modal, updateCountItem, refreshSessionUI, undo, pause/finish, uncounted items dialog (_showUncountedDialog, _markUncountedAsShortages). Bridge between session.js and camera.js |
...
| 32 | stock-count-report.js | modules/stock-count/stock-count-report.js | 275 | Diff report with per-item checkbox (approve/skip) + reason column for discrepancies, bulk toolbar, delegates unknown section to stock-count-unknown.js, _doConfirmCount (approved → RPC, skipped → status='skipped', reasons → saved, matched counted in total), cancelCount. Excel export moved to stock-count-export.js |
| 32b | stock-count-export.js | modules/stock-count/stock-count-export.js | 99 | Stock count Excel export: exportCountExcel (modal with diffs-only vs all + sort picker), _doExportCountExcel (sorted export with reason column) |
...
| 43 | debt-dashboard.js | modules/debt/debt-dashboard.js | 339 | Debt dashboard summary: loadDebtSummary (calls loadAgingReport + _loadPendingInvoiceBanner), loadAgingReport (5-bucket aging breakdown), _loadPendingInvoiceBanner (pending_invoice count badge), loadSuppliersTab (aggregated supplier table with debt/overdue/prepaid progress USED/TOTAL + hasReceiptDocs/hasHistory flags), renderSuppliersToolbar, renderSuppliersTable (prepaid column: green used / red total), openQuickOpeningBalance, openPaymentForSupplier |
| 43b | debt-supplier-filters.js | modules/debt/debt-supplier-filters.js | 102 | Supplier tab filter chips: renderSupplierFilterChips(), applySupplierFilters(), _setSupFilter(). 3 filter groups: type (defined/general), history (with/without), debt (has_debt/overdue). Client-side filtering on _supTabData |
| 44 | debt-documents.js | modules/debt/debt-documents.js | 347 | Documents tab: loadDocumentsTab, _loadDocFileCounts, renderDocumentsTable (source badges, checkbox column, multi-doc count badge + expand sub-row), _toggleDocSubRow, viewDocument, openPaymentForDocument, _attachFileToDoc (multi-file), closeAndRemoveModal, cancelDocument, openPrepaidDeductModal, _doPrepaidDeduct |
| 44b | debt-doc-new.js | modules/debt/debt-doc-new.js | 232 | New document modal (split from debt-documents.js): openNewDocumentModal, _pickNewDocFiles, _renderNewDocFileList, _removeNewDocFileAt, calcNewDocTotal, calcNewDocFromTotal, _ndAutoCalcDueDate, saveNewDocument (server-side duplicate check with override + multi-file + saveDocFile + auto-deduct from prepaid), generateDocInternalNumber, _autoDeductFromPrepaid (auto-deduct from active prepaid deal on manual doc creation) |
...
| 44d | debt-doc-actions.js | modules/debt/debt-doc-actions.js | 316 | Document action toolbar + file attach: _editDocAttachMore, _showAttachChoiceModal, _doAttachFiles, _buildDocActionToolbar (OCR, linking, folder assign, supplier change, change type, review toggle, delete), changeDocSupplier (PIN-verified supplier reassignment), assignToFolder, changeDocumentType, _softDeleteDocument, _togglePendingReview |
| 44d3 | debt-expense-folders.js | modules/debt/debt-expense-folders.js | 185 | Expense folders CRUD: loadExpenseFolders, getExpenseFolders, renderExpenseFolders, toggleExpenseFolders, addExpenseFolder, editExpenseFolder, deleteExpenseFolder, _toggleFolderActive. Cache: window._expenseFoldersCache |
| 44d4 | debt-general-invoices.js | modules/debt/debt-general-invoices.js | 137 | General invoices view (docs without supplier): toggleGeneralInvoicesView, loadGeneralInvoices (folder/month/status filters), renderGeneralInvoices (table with folder column + assign button), _applyGenInvFilters |
| 44d2 | debt-doc-edit.js | modules/debt/debt-doc-edit.js | 279 | Document edit modal: editDocument (all amounts always editable, status dropdown with valid transitions), _buildStatusOpts, _editDocCalc, saveDocumentEdits (saves status changes), _learnFromDocumentEdits |
...
| 48 | debt-payment-alloc.js | modules/debt/debt-payment-alloc.js | 337 | Payment wizard steps 3-4: document allocation with FIFO (pre-selected docs sorted first + highlighted), manual override, allocation summary with mismatch warning, PIN confirmation, _wizSavePayment (creates payment + allocations, updates document paid_amount/status, cascading settlement, rollback on failure), _cascadeSettlement (toast notification on auto-close) |
...
| 50b | debt-supplier-tabs.js | modules/debt/debt-supplier-tabs.js | 288 | Supplier detail sub-tab renderers: loadSupplierTimeline, _renderTimelineHtml, _showAllTimeline, loadSupplierDocuments (filter bar + multi-select checkboxes), _sdApplyFilters, _sdUpdateCount, _sdToggleDocCb, _sdToggleAll, _sdUpdateSelectBar, _sdPaySelected (opens wizard with preSelectedDocIds), _sdNewDoc, loadSupplierPayments, loadSupplierReturns |
...
| 52g | table-resize.js | shared/js/table-resize.js | 102 | Shared resizable columns + sticky scrollbar. TableResize.register(id, key). Supports: inv-table, rcpt-items-table, po-items-table, po-view-table, sc-items-table. Synced sticky scrollbar via .tbl-sticky-bar. CSS: shared/css/table.css |
...
| 57b | ai-learning-dashboard.js | modules/debt/ai/ai-learning-dashboard.js | 159 | AI learning dashboard tab: loadAILearningTab (summary cards + per-supplier table), _renderAILearningSummary (4 cards: total/learning/suggesting/auto), _renderAILearningTable (stage badges, accuracy bars, reset button), _resetSupplierLearning (PIN-verified reset) |
...
| 58b | debt-filter-utils.js | modules/debt/debt-filter-utils.js | 78 | Shared filter building blocks: MONTHS_HE, buildMonthOpts(), buildYearOpts(), buildMonthPickerHtml(prefix, onChangeFn), toggleMonthPicker(), monthPickerChanged(), applyDocFilterSet(). Reused by debt-doc-filters.js and debt-supplier-tabs.js |
| 59 | debt-doc-filters.js | modules/debt/debt-doc-filters.js | 348 | Advanced document filtering + recycle bin: collapsible 8-criteria filter panel (status, type, supplier, date range with month picker toggle, amount range, source), saved filter favorites (localStorage, max 5), filter count display. Recycle bin: _toggleRecycleBin (shows deleted docs), _renderRecycleBin (table with restore + days remaining), _restoreDocument (PIN-verified), _loadDeletedDocCount (badge) |
...
| EF3 | remove-background | supabase/functions/remove-background/index.ts | 123 | Background removal via remove.bg API. POST {image_base64, session_token} with anon key+apikey headers. Validates session_token against auth_sessions. Returns processed PNG as base64. Logs usage to activity_log. |
...
| `_downloadImage` | `(url, filename)` | Downloads image via fetch→blob→object URL with specified filename |
| `_showFullImage` | `(url)` | Opens full-screen overlay with image at full resolution, download button, ✕ close. Pinch-to-zoom on mobile |
...
| Function | Parameters | Description |
|----------|------------|-------------|
| `_bgRemoveStart` | `(imageUrl, onConfirm)` | Loads image, shows method choice dialog (AI vs Canvas) |
| `_bgShowChoiceDialog` | `(img, originalUrl, onConfirm)` | Modal with two buttons: AI מקצועי (remove.bg) and עיבוד מהיר (Canvas) |
| `_bgRunAI` | `(img, originalUrl, onConfirm)` | Calls remove-background Edge Function using prizma_auth_token. Shows loading, error on failure (no auto-fallback) |
| `_bgRunLocal` | `(img, originalUrl, onConfirm)` | Canvas path with threshold slider (30-100%), before/after comparison |
| `_bgShowResult` | `(originalUrl, blob, resultUrl, method, onConfirm)` | AI result: before/after comparison, confirm/cancel |
| `_bgProcessLocal` | `(img, thresholdPct, callback)` | Canvas flood-fill from 8 seed points, edge softening, outputs WEBP blob |
...
| `autoDeductPrepaid` | `(supplierId, docId, amount, tenantId)` | Async. Auto-deducts from active prepaid deal on document creation. Checks deal.total_remaining >= amount, calls increment_prepaid_used RPC, marks doc as paid. Returns true if deducted, false if no deal or insufficient balance |

### modules/goods-receipts/receipt-ocr-flow.js

| Function | Parameters | Description |
|---|---|---|
| `_rcptOcrShowCompareBtn` | `()` | Shows the compare button above items table |
| `_rcptOcrHideCompareBtn` | `()` | Hides the compare button |
| `_rcptOcrRunComparison` | `()` | Async. Triggers PO comparison from OCR data |
| `_rcptOcrCachedRescan` | `()` | Async. Re-scans without re-uploading file |
| `_rcptOcrShowPOChoiceModal` | `()` | Shows modal to choose between open POs |
| `_rcptOcrChoosePO` | `()` | Selects a PO from the choice modal |
| `_rcptOcrSkipPO` | `()` | Skips PO selection |
| `_rcptOcrLearnDocNum` | `(fv, supplierId)` | Learns doc number pattern from OCR for supplier |
...
| `exportReceiptBarcodes` | `(receiptId)` | Async. Fetches receipt items, expands qty>1 to individual rows, exports to Excel file (barcode/brand/model/color/size/cost) via SheetJS |
| `printReceiptBarcodes` | `(receiptId)` | Legacy alias for exportReceiptBarcodes |
...
receipt-ocr.js
  ...
  → listens: 'receipt-confirmed' event [receipt-confirm-items.js] (Phase 5e, refactored from monkey-patch)
