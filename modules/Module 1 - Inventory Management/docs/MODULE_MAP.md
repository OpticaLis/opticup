...
| 43 | debt-dashboard.js | modules/debt/debt-dashboard.js | 330 | Debt dashboard summary: loadDebtSummary (calls loadAgingReport + _loadPendingInvoiceBanner), loadAgingReport (5-bucket aging breakdown), _loadPendingInvoiceBanner (pending_invoice count badge), loadSuppliersTab (aggregated supplier table with debt/overdue/prepaid + hasReceiptDocs/hasHistory flags), renderSuppliersToolbar, renderSuppliersTable, openQuickOpeningBalance, openPaymentForSupplier |
...
| 44 | debt-documents.js | modules/debt/debt-documents.js | 311 | Documents tab: loadDocumentsTab, _loadDocFileCounts, renderDocumentsTable (source badges, checkbox column via showCheckboxes opt), viewDocument, openPaymentForDocument (opens wizard pre-filled for single doc), _attachFileToDoc (multi-file), closeAndRemoveModal, cancelDocument, openPrepaidDeductModal, _doPrepaidDeduct |
