...
| 19d | receipt-ocr.js | modules/goods-receipts/receipt-ocr.js | 324 | OCR integration: initReceiptOCR (injects scan button), _rcptOcrScan (upload + Edge Function call), _applyOCRToReceipt (auto-fill supplier via OcrSupplierMatch with confidence hints), _rcptOcrShowSupplierHint (green/yellow/red indicator), _rcptOcrFC/_rcptOcrAddConfDot (per-field confidence dots), _rcptOcrSuggestPO (PO auto-suggestion), _rcptOcrShowBanner, _rcptOcrPreviewDoc, _rcptOcrUpdateTemplate (header + item learning), receipt-confirmed listener (learns supplier alias + updates ai_has_po_pattern) |
