...
| 48 | debt-payment-alloc.js | modules/debt/debt-payment-alloc.js | 365 | Payment wizard steps 3-4: document allocation with FIFO, manual override, allocation summary with mismatch warning, PIN confirmation, _wizSavePayment (creates payment + allocations, updates document paid_amount/status, cascading settlement, rollback on failure), _cascadeSettlement (toast notification on auto-close) |
...
| 50b | debt-supplier-tabs.js | modules/debt/debt-supplier-tabs.js | 231 | Supplier detail sub-tab renderers: loadSupplierTimeline, _renderTimelineHtml, _showAllTimeline, loadSupplierDocuments (with inline filter bar: status, type, month picker, amount range), _sdApplyFilters, _sdUpdateCount, _sdNewDoc, loadSupplierPayments, loadSupplierReturns |
