...
| 17 | goods-receipt.js | modules/goods-receipts/goods-receipt.js | 349 | Receipt list: loadReceiptTab, loadPOsForSupplier, onReceiptPoSelected (ONE row per PO item with qty=remaining, matches existing inventory), updatePOStatusAfterReceipt (filters out not_received/returned items), openNewReceipt, _initRcptSupplierSelect (searchable supplier dropdown via createSearchSelect) |
| 18 | receipt-form.js | modules/goods-receipts/receipt-form.js | 347 | Receipt form setup: file attachment state (_pendingReceiptFiles), multi-doc numbers (_rcptExtraNums, _rcptDocAmounts, _addRcptExtraNum, _removeRcptExtraNum, _onDocAmountChange, _renderRcptExtraNums, getRcptDocumentNumbers, getRcptDocAmounts), _pickReceiptFile, _initReceiptDropzone, _stageReceiptFile, _renderReceiptFileList, _resetReceiptAttachUI, _removeReceiptFileAt, _removeReceiptFile, openExistingReceipt, toggleReceiptFormInputs, searchReceiptBarcode |
