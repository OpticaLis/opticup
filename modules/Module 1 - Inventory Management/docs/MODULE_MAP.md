...
| 44 | debt-documents.js | modules/debt/debt-documents.js | 350 | Documents tab: loadDocumentsTab (fetch docs+types+suppliers), renderDocFilterBar, applyDocFilters, renderDocumentsTable (with upload tooltip), viewDocument (delegates to editDocument), _attachFileToDoc, openNewDocumentModal, saveNewDocument, generateDocInternalNumber, cancelDocument |
| 44b | debt-doc-edit.js | modules/debt/debt-doc-edit.js | 207 | Document edit modal with AI learning + OCR items: editDocument (editable form + OCR items table from ocr_extractions), _editDocCalc (auto-calc VAT/total), saveDocumentEdits (validate+save+AI learn), _learnFromDocumentEdits (compare OCR vs edits, call updateOCRTemplate) |
...
| 50 | debt-supplier-detail.js | modules/debt/debt-supplier-detail.js | 342 | Supplier detail view: openSupplierDetail (slide-in panel with summary + 4 sub-tabs), closeSupplierDetail, loadSupplierTimeline (merged docs+payments sorted by date), loadSupplierDocuments (filtered table), loadSupplierPayments (filtered table), loadSupplierReturns (delegates to debt-returns.js) |
