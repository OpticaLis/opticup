...
| 18a | receipt-form-items.js | modules/goods-receipts/receipt-form-items.js | 334 | Receipt item row management: addReceiptItemRow (note row, editable model/size/color on existing items), getReceiptItems (includes note), _toggleRcptNote, _onReceiptStatusChange, updateReceiptItemsStats, addNewReceiptRow, generateReceiptBarcodes |
| 18a2 | receipt-form-validate.js | modules/goods-receipts/receipt-form-validate.js | 59 | Receipt form UI validation (sort lock + invoice-total compare): _rcptSortLocked state, gated column-sort handler, toggleRcptSortLock, _initRcptSortLockUI |
