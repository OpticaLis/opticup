...
| 18a | receipt-form-items.js | modules/goods-receipts/receipt-form-items.js | 284 | Receipt item row management: addReceiptItemRow (product_type dropdown), getReceiptItems (single barcode, product_type), _onReceiptStatusChange (moves not-received to bottom), updateReceiptItemsStats (total cost summary), addNewReceiptRow, generateReceiptBarcodes |
...
| 19 | receipt-actions.js | modules/goods-receipts/receipt-actions.js | 192 | Receipt save/cancel: saveReceiptDraft (persists barcodes_csv+ordered_qty+receipt_status+from_po+product_type), saveReceiptDraftInternal (same), cancelReceipt, backToReceiptList |
| 19b | receipt-confirm-items.js | modules/goods-receipts/receipt-confirm-items.js | 187 | Item-level inventory processing: confirmReceiptCore (one barcode per product line, creates 1 inventory record with full qty, increments existing by effectiveQty, rollback on failure, auto-create supplier doc), createNewInventoryFromReceiptItem (uses item qty + product_type) |
