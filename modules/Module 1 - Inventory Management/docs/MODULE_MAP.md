...
| 19f | receipt-po-compare.js | modules/goods-receipts/receipt-po-compare.js | 238 | PO comparison report: _poCompBuildReport (match by brand\|model\|size\|color key), _poCompShowReport (5-section modal: matched/shortage/priceGap/notInPo/missing), _poCompCollectDecisions, _poCompApplyDecisions (write price_decision/po_match_status, auto-create supplier_return for rejected items), _poCompLearnPricePattern (VAT detection) |
...
| 44b | debt-doc-new.js | modules/debt/debt-doc-new.js | 196 | New document modal (split from debt-documents.js): openNewDocumentModal, _pickNewDocFiles, _renderNewDocFileList, _removeNewDocFileAt, calcNewDocTotal, calcNewDocFromTotal, _ndAutoCalcDueDate, saveNewDocument (multi-file + saveDocFile), generateDocInternalNumber |
...
| 44d | debt-doc-edit.js | modules/debt/debt-doc-edit.js | 296 | Document edit modal: editDocument (editable form + file gallery + editable items), _editDocAttachMore (choice modal: save only vs save+OCR), _showAttachChoiceModal, _doAttachFiles, _editDocCalc, saveDocumentEdits (+_saveEditedItems), _learnFromDocumentEdits |
...
| 50 | debt-supplier-detail.js | modules/debt/debt-supplier-detail.js | 387 | Supplier detail view: openSupplierDetail, closeSupplierDetail, _switchDetailTab, loadSupplierTimeline, loadSupplierDocuments (reuses renderDocumentsTable with hideSupplierCol + all action buttons), _sdNewDoc (pre-fills supplier), loadSupplierPayments, loadSupplierReturns, openSetOpeningBalance, _saveOpeningBalance, _savePaymentTerms |
...
| 54 | ai-ocr.js | modules/debt/ai/ai-ocr.js | 346 | OCR trigger, review screen, correction flow with learning: triggerOCR (calls ocr-extract Edge Function), showOCRReview (side-by-side modal + supplier OCR stats), _ocrSave (saves corrections + creates/updates existing supplier_document + updates OCR template), _ocrCurrentFileUrl (global: tracks file for update-vs-insert logic), confidence indicators per field, _injectOCRScanIcons (adds scan buttons to doc table rows), _injectOCRToolbarBtn (toolbar scan button), patches loadDocumentsTab |
