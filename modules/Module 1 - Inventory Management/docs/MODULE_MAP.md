...
| 14 | po-items.js | modules/purchasing/po-items.js | 342 | PO item management: renderPOItemsTable, cascading brand→model→color/size datalists, addPOItemManual, addPOItemByBarcode, removePOItem, duplicatePOItem, updatePOTotals, _calcPOFinalPrice, _onPOFinalPriceChange, _onPODiscountChange, _onPOCostChange, _updatePOStockCounter, _filterPOByBrand, _updatePOBrandFilter |
...
| 16b | po-view.js | modules/purchasing/po-view.js | 249 | PO view with received qty tracking, receipt-based reason badges, editable sell price+discount columns (sent/partial POs), _savePOViewPrices, summary row (lines/units/total), PO event delegation |
...
| 18a | receipt-form-items.js | modules/goods-receipts/receipt-form-items.js | 279 | Receipt item row management: addReceiptItemRow, getReceiptItems (single barcode per product line), _onReceiptStatusChange (moves not-received to bottom), updateReceiptItemsStats (total cost summary), addNewReceiptRow, generateReceiptBarcodes (ONE barcode per product line) |
...
| 19b | receipt-confirm-items.js | modules/goods-receipts/receipt-confirm-items.js | 188 | Item-level inventory processing: confirmReceiptCore (one barcode per product line, creates 1 inventory record with full qty, increments existing by effectiveQty, rollback on failure, auto-create supplier doc), createNewInventoryFromReceiptItem (uses item qty) |
