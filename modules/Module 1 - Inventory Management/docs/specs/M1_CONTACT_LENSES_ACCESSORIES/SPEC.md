...
- **C-B1** (2026-05-16T~17:20Z): accessory schema applied via Supabase MCP `apply_migration name=m1_accessory_schema_part_b`. Stage 3 Part B complete. Created 3 tables (accessory_variant 14-col / tenant_accessory_stock 6-col / accessory_variant_display_seq 3-col global singleton) + 6 RLS policies (3+2+1 same pattern as contact lens) + 1 RPC (next_accessory_variant_display_id, AC- prefix) + 4 indexes. Total new indexes Part A + Part B = 8 (matches SPEC §3 S14 exactly). accessory_variant follows lens-pattern (owner_tenant_id + is_published + lifecycle_status + is_deleted). Iron Rule 18 UNIQUE on sku per-owner via coalesce. Prizma row counts unchanged (inventory=8894, lens_design=1). Stage 2 schema work (Parts A + B) COMPLETE.
- **C-C1+C-C2** (2026-05-16T~17:35Z, COMBINED per §9 #8 commit reordering autonomy — HTML IDs + JS handlers are tightly coupled). Stage 4 Part C UI infrastructure: `modules/inventory/inventory-shell.js` extended (215→324 lines, +109 lines under Rule 12 cap) to activate the two previously-disabled sidebar categories (contact-lenses + accessories) and route to per-category loaders. `inventory.html` extended (1156→1200 lines, +44 lines): added 2 nav strips (`#contactNav`, `#accessoryNav`, hidden by default, 6 buttons each with permission attrs); added 12 section shells (6 `.contact-tab-section` + 6 `.accessory-tab-section`) before `</main>`; added 2 script tags for the new loaders before `inventory-shell.js`. Per DG-5.A parallel prefix isolation: contact tabs use `data-contact-tab`, accessory tabs use `data-accessory-tab` — zero collision with existing `data-lens-tab` handlers. Loaders (InvShellContact, InvShellAccessory) are referenced but undefined at this commit; full definition lands in C-C3.
- **C-C3+C-C4+C-C5** (2026-05-16T~18:00Z, BUNDLED per §9 #8 commit reordering autonomy — UI module layer + CSS + permission seed are a coherent "make the new categories work" concern). 26 new files + 1 CSS extension + 1 permission migration. Files: (a) 2 loaders `inventory-shell-contact.js` (208 lines) + `inventory-shell-accessory.js` (200 lines), each mirror of inventory-shell-lens.js with parallel-prefix isolation per DG-5.A; (b) 12 partial HTMLs at `modules/contact-lens-<sub>/contact-lens-<sub>-partial.html` and `modules/accessory-<sub>/accessory-<sub>-partial.html` — inventory tabs have real grid+filter UI (~30 lines each, ID-prefixed `cl-` / `ac-`); other 10 tabs are MV placeholders with permission-gate + "screen coming soon" empty state; (c) 12 module JS files: contact-lens-inventory.js + accessory-inventory.js have real DB queries (loads variants from contact_lens_variant/accessory_variant joined with lens_design/lens_brand, loads stock from tenant_*_stock filtered by getTenantId(), renders table); other 10 are MV placeholders that gate + show empty state; (d) `css/lens-tabs.css` extended +43 lines with 2 alias selectors for `#contactNav, #accessoryNav` buttons (inherit visual primitives from `#lensNav` rules via duplicated selector blocks — Iron Rule 21 fine since CSS doesn't have selector aliases without preprocessing); (e) MCP migration `m1_contact_lens_accessory_permission_seed` inserted 24 rows in `permissions` (12 keys × 2 tenants) + 60 rows in `role_permissions` (ceo+manager get all 12 per tenant = 48; team_lead+viewer+worker get only `.inventory.view` per tenant = 12; total 60). All files under Rule 12 350-line cap. Prizma `inventory`/`lens_design`/`goods_receipt_items` row counts unchanged post-seed (8894/1/275). In-flight decision D-2: Prizma permissions+role_permissions write authorized by Brief §2.3 + Success Criterion #12 explicit instruction — distinct from §6 "no Prizma data writes" which targets operational/inventory tables.
- **C-D1** (2026-05-16T~18:25Z, after one v1 rollback for `tenant_lens_stock.location_id NOT NULL` discovery → v2 success). Sample lens catalog seed for DEMO via Supabase MCP `apply_migration name=m1_demo_seed_lens_sample_catalog_v2`. INSERTed: 5 platform brands (Hoya/Essilor/Zeiss/Nikon/Rodenstock) + 10 lens_design (2 per brand: vision+progressive) + 30 lens_variant (3 refractive_index per design: 1.50/1.60/1.67) + 30 tenant_lens_stock rows (first 10 variants × 2 location rows = 20 NEW; existing 10 stayed = 30 total demo) + 2 demo POs (PO-100001 sent + PO-100002 fully_received) with 5 PO lines total. Bumped `lens_variant_display_seq.last_value` from 2 → 32. Demo `purchase_order` count: 6 → 8.
- **C-D-CORRECTIVE** (2026-05-16T~18:35Z, before C-D2). Corrective migration `m1_corrective_drop_po_line_variant_fks`: DROP CONSTRAINT `purchase_order_line_variant_id_fkey` + DROP CONSTRAINT `purchase_receipt_line_variant_id_fkey`. SPEC §0.C F-DB-5 INCORRECTLY claimed these tables had no FK on variant_id; empirical FK probe at SPEC seal time missed them. Both had hard FK → lens_variant(id). Per SPEC §9 #10 INTENT-vs-LITERAL + corollary-edit pattern (P-AUTHOR-1 from M1_INVENTORY_UNIFIED_SCREEN), polymorphic variant_id routing requires FK removal. Sibling to SPEC §4 #3 supplier_catalog_offering FK drop. Counts as SPEC §4 destructive ops corollary; documented as in-flight decision D-4. SPEC §0.C F-DB-5 amended in-place: the FKs DID exist; this corrective migration resolves them.
- **C-D2** (2026-05-16T~18:40Z, after CHECK+FK retries → v3 success). Sample contact-lens catalog seed for DEMO via Supabase MCP `apply_migration name=m1_demo_seed_contact_lens_v3`. INSERTed: 5 platform CL brands (Acuvue/Bausch+Lomb/CooperVision/Alcon/Ciba) + 10 lens_design (2 per brand: Daily/Monthly) with product_type='contact_lens' + 40 contact_lens_variant rows (4 SPH steps × 10 designs; alternating daily/monthly wearing_schedule) + 20 tenant_contact_stock rows on demo (first 10 variants × 2 location rows, with some near-expiry dates for Brief §2.4 expiry-warning exercise) + 2 demo POs (PO-200001 sent + PO-200002 fully_received) with 5 PO lines total. Bumped `contact_lens_variant_display_seq.last_value` from 0 → 40. Demo `purchase_order` count: 8 → 10. **In-flight D-3** (FINDING F-2 LOW): `lens_design.lens_type` CHECK doesn't include `'soft_contact'` so all 10 CL designs use `'single_vision'` as semantic stand-in. Acceptable for sandbox demo; CHECK expansion deferred to follow-up SPEC. **In-flight D-4** above (FK drops on PO line variant_id).
- **C-D3** (2026-05-16T~18:50Z, single-attempt success). Sample accessory catalog seed for DEMO via Supabase MCP `apply_migration name=m1_demo_seed_accessory_sample_catalog`. INSERTed: 5 platform accessory brands (Zeiss-Accessories — name-suffixed to avoid dedup against C-D1's Zeiss; Rayban/Warby/Crizal/Persol) + 25 lens_design rows (5 brands × 5 categories: Cases/Cloths/Cleaning/Repair/Cords) with product_type='accessory' + 25 accessory_variant rows + 30 tenant_accessory_stock rows on demo (first 15 variants × 2 locations) + 2 demo POs (PO-300001 partial + PO-300002 fully_received). PO-300001 includes one variant-less manual line (source='manual', manual_description set) — exercises F-2 path per Brief §2.4. Demo `purchase_order` count: 10 → 12 (matches SPEC §3 S26 ≥ 12). **Stage 5 Part D COMPLETE. Demo totals: 30 lens + 40 CL + 25 accessory variants = 95 variants across 3 categories.**
- **C-D-PRIZMA-DELTA-VERIFICATION** (2026-05-16T~18:55Z, post-Stage-5): re-ran §0.E Prizma baseline query. ALL 27 tables UNCHANGED: inventory=8894, brands=232, goods_receipt_items=275, purchase_order=0, purchase_orders=3, stock_count_items=7297, change_approval_log=0, tenant_lens_stock=0, tenant_contact_stock=0, tenant_accessory_stock=0, suppliers=38. **Prizma row-count delta = 0 across ALL probed tables. SPEC §3 S32 PASS at mid-Pipeline.**
- **C-FIX-1** (2026-05-16T~18:05Z, Stage 8b fix loop). Stage 8 Localhost-Tester T-FAIL-1 fix: removed stale `disabled` class + `title="בקרוב"` + `<span class="inv-cat-badge">בקרוב</span>` from inventory.html sidebar entries for `contact-lenses` and `accessories`; added `data-permission="contact_lens.inventory.view"` and `data-permission="accessory.inventory.view"` respectively. 4-line semantic patch in inventory.html:51-58. Click handler `bindSidebarClicks` (inventory-shell.js:155-162) now reaches `setActiveCategory` on click — the loader pipeline (already proven functional via T-B.PROGRAMMATIC) is now wired to the user entry point. SPEC §3 S15 now PASS. Corollary-edit pattern (P-AUTHOR-1 from M1_INVENTORY_UNIFIED_SCREEN) advances from 1/3 → 2/3 firings.
