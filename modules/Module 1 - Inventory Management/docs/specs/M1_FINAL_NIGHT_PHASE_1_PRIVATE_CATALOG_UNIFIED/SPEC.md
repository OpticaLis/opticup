...
- **C-6 ✅** — 2026-05-17 night — In-flight fix: brand-level product_type filter in `shared/js/catalog-private-admin.js` `loadBrands()`. Smoke test caught: all 16 global brands rendered in Lens tab even though Acuvue/Alcon/etc. are contact-lens-only. Root cause: my MVP design filtered designs+variants by product_type but not brands. Fix: on Global sub-tab, first fetch distinct `brand_id` from `lens_design WHERE product_type=opts.productType AND owner_tenant_id IS NULL`, then filter brand query with `.in('id', brandIds)`. Private sub-tab continues to show all tenant private brands (regardless of designs — supports "create brand first, then add designs" flow). Verified via Chrome MCP: Glasses → 6 brands (Essilor/Hoya/Nikon/Rodenstock/SmokeBrand_M1A/Zeiss); Contact-lens → 5 brands (Acuvue/Alcon/Bausch+Lomb/Ciba/CooperVision); Accessory → 5 brands (Crizal/Persol/Rayban/Warby/Zeiss-Accessories). Matches DB-side product_type partitioning exactly.

- **C-8 ✅ (Phase 1-FIX)** — 2026-05-17 — Root cause fix for "tab buttons hidden despite seeded perms" (Daniel manual VFV caught: lens tab missing, contact+accessory showed `המסך יופיע בהמשך` placeholder from existing catalog-admin tab — the new private-catalog tab buttons were `display:none` because of an OR-parsing bug). **Root cause:** `js/auth-service.js:313-315` in `applyUIPermissions()` directly called `hasPermission(el.getAttribute('data-tab-permission'))` with the raw attribute value — but my new tab perms use the `|` OR syntax (`lens.catalog.private.manage|lens.catalog.global.view`). `hasPermission('a|b')` returns false (the literal string isn't a valid key), so the loop set `display:none` on all buttons using OR syntax — overriding `PermissionUI.apply()`'s correct OR-parsed result. **Fix:** Added `_checkPermStrAuth(permStr)` helper that splits on `|` and runs `hasPermission()` per part. Both `[data-permission]` + `[data-tab-permission]` loops in `applyUIPermissions()` now use it. Auto-fixes ANY current/future button with `|` OR syntax. Also added defensive `PermissionUI.refresh(root)` + `PermissionUI.refreshTo(container)` to `shared/js/permission-ui.js` (clears prior inline display:none, re-scans) — for callers who explicitly need re-evaluation after async perm cache refresh. **Verified via Chrome MCP fresh-reload (no manual intervention):** Lens/Contact/Accessory all show "📚 הקטלוג שלי" button auto-visible after sidebar category click. Full drill-down works: brand → designs → variants → detail pane. CRUD verified: 3 test brands seeded via real Add-Brand flow (prompt dialog) on demo tenant: `אופטיקה אורית — אביזרים` (`bac58d89...`), `אופטיקה אורית — עדשות` (`2506ca1d...`), `אופטיקה אורית — עדשות מגע` (`25c8dccc...`). Prizma row counts: lens_brand=0, lens_design=0, lens_variant=0. RLS verified at policy level: new rows are `is_published=false, lifecycle_status='draft'` (excluded from public_view); `owner_view USING owner_tenant_id=jwt.tenant_id` correctly isolates per-tenant. Screenshots: `_archive/m1-final-completion-2026-05-17/phase1fix_lens_private_catalog_real_user_path.png` + `phase1fix_contact_private_with_3_test_brands.png`.
