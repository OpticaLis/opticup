...
2. **🟡 PARTIAL CLOSE 2026-05-15 — 17 of 35 public views lack `security_invoker=on` (49%).** Includes EVERY storefront-facing view (`v_storefront_blog_posts`, `v_storefront_products`, etc.) + `v_ai_content`, `v_content_translations`, `v_crm_event_stats`, `v_public_tenant`. Repeat of the exact `SECURITY_HOTFIX_2026_05_13` class — that hotfix patched a subset; this is the long tail. Suggested SPEC: `M1_5_VIEW_SECURITY_INVOKER_SWEEP`. — **2 of 17 closed in `SECURITY_HOTFIX_2_2026_05_15` §1.2 at commit `40cde93` (`v_storefront_reviews`, `v_storefront_components` — the only two whose base tables have anon-friendly RLS). 15 deferred to mandatory follow-up `SECURITY_HOTFIX_3` (Brief next session) — they require base-table RLS expansion on `blog_posts`, `storefront_pages`, `ai_content` before the flip is safe; `security_invoker=on` without that prerequisite would dark the storefront on anon callers per RESOLVED escalation 2026-05-15T1110Z.**

3. **🟢 RESOLVED FOR IN-SCOPE SUBSET 2026-05-15 — 20 SECURITY DEFINER functions take `p_tenant_id` without JWT verification.** Higher-risk subset (anon-callable): `generate_daily_alerts`, `get_po_aggregates`, `get_translation_context`, `is_feature_enabled`, `check_plan_limit`, `_record_touchpoint`, `verify_campaign_page_password`. Mitigated today by service_role-only grants on tenant-management fns, but defense-in-depth missing. Suggested SPEC: `M1_5_RPC_TENANT_JWT_VALIDATION`. — **24 of 24 in-scope SECURITY DEFINER RPCs (pre-flight surfaced 24, not 20 — 4 more functions met the criteria) hardened in `SECURITY_HOTFIX_2_2026_05_15` §1.3 at commit `40cde93` with 3-role-aware Block A (service_role bypass + strict for non-service-role; 23 RPCs) + Block A-alt slug-anchor (`verify_campaign_page_password`, 1 RPC). 16 Option B anon EXECUTE revoked; 7 already-non-anon-callable; 1 Option A retained anon EXECUTE with slug check. Verified Stage 2 with WRONG-tenant JWT → 42501 on 2/2 sampled RPCs. The 15 pre-existing `anon_security_definer_function_executable` advisor findings on UNRELATED SECURITY DEFINER carry RPCs (`acknowledge_failed_messages`, `attendee_status_change_event_fn`, `event_status_change_event_fn`, `event_status_close_recycle_leads_fn`, `get_all_tenants_overview`, `increment_paid_amount`, `increment_prepaid_used`, `is_platform_super_admin`, `lead_status_change_event_fn`, `mark_translations_stale`, `promote_lead_on_message_sent`, `promote_to_platform`, `register_lead_to_event`, `resolve_touchpoints_to_lead`, `validate_slug`) are out-of-scope for HOTFIX_2 and folded into `SECURITY_HOTFIX_3` scope.**
