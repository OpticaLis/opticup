...
| 9 | **CRITICAL → 🟢 RESOLVED (2026-05-15)** | 20 SECURITY DEFINER fns missing JWT validation | Closed for 24-RPC in-scope subset via `SECURITY_HOTFIX_2_2026_05_15` §1.3 at SHA `40cde93`. **15 pre-existing carry RPCs closed via `SECURITY_HOTFIX_3_2026_05_15` §1.5 at SHA `e64f9c9` (14 Option B + 1 Option C `validate_slug`). 5 RPCs received NEW Block A bodies (`increment_paid_amount`, `increment_prepaid_used`, `mark_translations_stale` via JOIN-derived tenant; `register_lead_to_event` + `resolve_touchpoints_to_lead` upgraded from weaker variants). F-CRIT-3 advisor 17→2 (remaining: `validate_slug` + `verify_campaign_page_password`, both intentional).** | L | RESOLVED |
| 10 | HIGH → 🟢 RESOLVED (2026-05-15) | 17 SECURITY DEFINER fns missing `SET search_path` | `sync_lead_status_from_attendee` closed via HOTFIX_2 §1.1 at SHA `40cde93`. 7 of 24 in-scope RPCs in HOTFIX_2 §1.3 collaterally hardened. **3 carry RPCs (`is_platform_super_admin`, `promote_to_platform`, `promote_lead_on_message_sent`) received `SET search_path TO 'public'` via HOTFIX_3 §1.5 at SHA `e64f9c9`. 1 more (`save_translation_memory_batch` 2nd overload) via HOTFIX_3 §1.4 at SHA `a20343a`. Plus 5 RPCs got NEW search_path via the same commit as their Block A rewrites. `function_search_path_mutable` advisor 23→16.** | S | RESOLVED |
| 11 | MED → 🟢 RESOLVED (2026-05-15) | `is_platform_super_admin()` PUBLIC EXECUTE + no search_path | Highest individual-function hijack risk. **Closed via HOTFIX_3 §1.5 at SHA `e64f9c9` — REVOKE EXECUTE FROM anon + GRANT to authenticated/service_role + `SET search_path TO 'public'`. The `auth.uid()` body check remains as the authentication primitive.** | XS | RESOLVED |
| 12 | MED → 🟢 RESOLVED (2026-05-15) | `save_translation_memory_batch` has 2 overloads | Rule 21 violation. **1st overload (`p_tenant_id uuid, p_entries jsonb`) hardened in HOTFIX_2 §1.3 at SHA `40cde93`. 2nd overload (`p_entries jsonb`) hardened via HOTFIX_3 §1.4 at SHA `a20343a` — 3-role-aware Block A adapted for entry-level tenant_id (FOR EACH entry: JWT tenant must match entry.tenant_id, service_role bypasses) + SET search_path + REVOKE anon EXECUTE.** | S | RESOLVED |
