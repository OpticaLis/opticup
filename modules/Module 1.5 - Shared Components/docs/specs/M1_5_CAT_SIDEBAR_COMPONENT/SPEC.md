...
- **C2** (2026-05-17T~09:05Z): refactored `inventory.html` to consume component. 5 corollary edits per DG-5: (1) added `<link rel="stylesheet" href="shared/css/cat-sidebar.css">` in head; (2) dropped `class="has-inv-sidebar"` from body; (3) wrapped body content in `<div class="cat-sidebar-host"><div class="main-content">...</div><div id="cat-sidebar-mount"></div></div>` (modals + scripts outside wrapper — they don't need grid layout); (4) removed inline `<aside id="inv-sidebar">...</aside>` block (37 lines) + replaced with mount slot near body end; (5) added `<script type="module">import { initCatSidebar } from './shared/js/cat-sidebar.js'; initCatSidebar({...config...});</script>` before existing classic `<script src=...>` tags. 0 NUL bytes. Line count 1200 (vs SPEC S11 prediction ~1178±5 — slight over-prediction; script block included permission attrs + comments + Unicode-escaped icons, ended at ~28 lines vs SPEC's ~15-line estimate. INTENT-vs-LITERAL absorbed; D-1 documented).
- **C3** (2026-05-17T~09:15Z): pruned `css/inventory-shell.css` to remove sidebar-specific rules (extracted to `shared/css/cat-sidebar.css`) + removed brittle per-element overlap selector list. 108 lines removed (248 → 140). Verifications: `has-inv-sidebar` 0 hits, `#inv-sidebar` 0 hits, `.inv-cat-item` 0 hits (all extracted). Kept: `.supplier-cat-badge` (3 hits), `.ul-filter-bar` (5 hits), `lens-tab-section` (4 hits) — cross-cutting non-sidebar rules. 0 NUL bytes. The brittle `body.has-inv-sidebar > main, > #mainNav, > #lensNav, > #low-stock-banner` selector list (the one that didn't cover contactNav + accessoryNav — Daniel's bug) is GONE. Structural grid rule in cat-sidebar.css `.cat-sidebar-host { display: grid; grid-template-columns: 1fr 240px; }` now protects all current + future nav strips uniformly.
- **C4** (2026-05-17T~09:20Z): `docs/GLOBAL_MAP.md` §5.4 (Key JS globals ERP) extended with `initCatSidebar(config)` entry — bundled the CSS class info into the same registry line (CSS classes don't get their own table in GLOBAL_MAP today). Entry explicitly flags the ES Module divergence + companion cat-sidebar.css. No destructive ops; docs only.
