...
- **C1** (2026-05-17T~19:25Z): swapped `grid-template-columns` column order in `shared/css/cat-sidebar.css` from buggy `1fr var(--cat-sidebar-width, 240px)` (RTL-mismatched) to correct `var(--cat-sidebar-width, 240px) 1fr` (sidebar slot first = inline-start = RIGHT in RTL = matches fixed sidebar). Added ~22-line explanatory comment block above the rule warning future maintainers not to swap back. File size 162 → 185 lines (+23: 1-line rule edit + 22 comment lines). 0 NUL bytes. Verifications: S1 (correct order present) = 1; S2 (buggy order absent) = 0; S3 (RTL comment present) = 8. Mobile @media (max-width: 800px) block untouched per §4 explicit restriction. Single file modified per §9 autonomy envelope. **OUTCOME: hypothesis FAILED — Tester VFV showed all 8 surfaces 🔴 with main-content collapsed to 240px under the fixed sidebar (grid auto-placement uses DOM child order, not config order; the swap landed .main-content in the wrong cell). See TEST_REPORT.md (RED) + escalation `2026-05-17T1945Z_C1_HYPOTHESIS_FAILED.md`.**
- **C2** (2026-05-17T~20:05Z): replaced grid-based approach entirely with Option A (Daniel-approved). `shared/css/cat-sidebar.css`: dropped `display: grid; grid-template-columns` from `.cat-sidebar-host` (now `display: block`); dropped `#cat-sidebar-mount { width: 240px }` rule (irrelevant once sidebar is position:fixed without a grid cell); added `margin-inline-start: var(--cat-sidebar-width, 240px)` to `.cat-sidebar-host > .main-content` (RTL flips automatically → reserves 240px on the RIGHT edge for the fixed sidebar). Mobile @media block: dropped `grid-template-columns: 1fr` + mount `width: 100%` rules; added `margin-inline-start: 0` reset on `.main-content` to restore full width on narrow viewports. Replaced 22-line C1 RTL-grid comment block with new ~18-line Option A rationale. Refreshed file-header comment to drop "CSS grid wrapper" language. File size 185 → 161 lines (-24, net). 0 NUL bytes. Verifications: `grid-template-columns` = 0 hits; `display: grid` = 0 hits; `margin-inline-start: var(--cat-sidebar-width` = 1 hit; `margin-inline-start: 0` (mobile reset) = 1 hit. Single file modified per §9 autonomy envelope. `.main-content` selector verified to exist in `inventory.html:40` inside `.cat-sidebar-host` (no rename needed per dispatch contingency).
