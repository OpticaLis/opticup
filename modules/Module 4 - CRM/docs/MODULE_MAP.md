...
| `crm-confirm-send.js` | 271 | **[P20 + P21 + P29]** Confirmation Gate shown before any automated send. **[P21]** Redesigned to 2-tab layout: Messages tab (1 card per channel per rule with representative body preview) + Recipients tab (sortable paginated table, 50/page, dedup by lead_id with ×N badge when same lead is targeted by multiple rules). Approve → `CrmMessaging.sendMessage` per plan item; Cancel → inserts `pending_review` rows into `crm_message_log`. **[P29]** `writePendingReviewRows` now includes `run_id: it.run_id || null` in the inserted row so abandoned runs link drill-down ↔ pending_review. Exports `window.CrmConfirmSend.show(sendPlan)`. |
