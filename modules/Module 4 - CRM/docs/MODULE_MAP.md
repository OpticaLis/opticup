...
> **Last updated:** 2026-04-30 (P29 — automation runs observability: `crm_automation_runs.updated_at` + auto-update trigger; reaper block in `dispatch-queue` EF flips runs >1h old to `aborted`; `run_id` now written on `pending_review` message_log rows; drill-down modal renders run-status header + state-aware empty-states; 2 stuck Prizma rows backfilled to `aborted`)
