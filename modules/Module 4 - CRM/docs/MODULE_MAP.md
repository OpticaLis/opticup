...
> **Last updated:** 2026-04-29 (P23 — attendee cancellation flow; 2 new files: crm-event-day-coupon.js, crm-attendee-cancel.js; coupon dispatch lifecycle guards; refunds-pending dashboard banner)
...
| `crm-attendee-cancel.js` | 141 | **[P23]** Attendee cancellation dialog. Three paths chosen by `payment_status`: `paid` opens a 2-button choice modal ("מגיע החזר" → status='cancelled' + payment_status='refund_requested' with timestamps; "לא מגיע החזר" → payment_status='no_refund_due' only — coupon NOT freed); everything else opens a simple confirm modal that sets status='cancelled' + cancelled_at=now(). Every UPDATE filters by tenant_id (Rule 22). `cancelButtonHtml(attendee)` returns empty string for already-cancelled attendees so the button auto-hides. `openCancelDialog(attendeeId, opts)` accepts `opts.onAfterCancel(attendeeId)` for caller-driven row refresh. Local helpers `_cancelToast` / `_logCancel` uniquely named. Exports `window.CrmAttendeeCancel`. |
