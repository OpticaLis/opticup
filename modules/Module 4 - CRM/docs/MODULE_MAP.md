...
| `crm-dashboard.js` | 336 | **[B8 + P23]** Dashboard via Tailwind: 4 gradient KPI cards (indigo/cyan/emerald/amber) with per-variant sparkline bars, 3-column alert strip, gradient stacked bar chart, 3 conic-gradient gauges (inline style), animate-pulse activity feed, horizontal timeline cards with progress bars. **[P23]** `loadRefundsBanner()` queries `crm_event_attendees` for `payment_status='refund_requested'` and renders an amber `[data-banner="refunds-pending"]` banner above the KPI cards when count>0 (hidden when 0). Click opens `openRefundsModal(rows)` listing all pending refunds; row click routes to lead detail via `openCrmLeadDetail`. |
| `crm-leads-tab.js` | 348 | **[B8 + P2a + P31 commits 0a/5]** Leads via Tailwind: white-card table, indigo filter chips, indigo bulk bar (now with working "שנה סטטוס" → `CrmLeadActions.openBulkStatusPicker`), pagination, delegates kanban + cards to `crm-leads-views.js`. Exports `reloadCrmLeadsTab`. **[P31 0a]** Filter-chip + pagination render extracted to `crm-leads-tab-filters.js`. **[P31 5]** New `loadFailedCounts()` aggregates `crm_message_log.status='failed'` per lead in last 90 days; rows render `⚠️N` badge in red beside lead name when count>0; rose toggle pill `📩 הודעות כושלות (M)` appears in chip bar when M>0; clicking pill sets `_failuresOnly` flag that restricts the filtered set; `window.reloadCrmLeadsFailedCounts` exposed for retry-after callbacks. |
...
| `crm-leads-detail.js` | 332 | **[B8 + P2a + P31 commits 0b/6]** Lead detail modal via Tailwind: gradient-avatar header + clickable status badge → tier-filtered dropdown (P2a) + 5 underline tabs (events/messages/notes/timeline/details) — notes tab has textarea + "הוסף" button at top (P2a) + 4 gradient action buttons. Falls back to `getCrmIncomingLeadById` when `getCrmLeadById` misses. **[P31 0b]** Messages-tab fetch + render extracted to `crm-leads-detail-messages.js` (channel/status const maps + per-lead message_log SELECT + renderMessagesList). **[P31 6]** Renders failed-messages collapsible section above the avatar header (only when N>0); `renderAndWire(body, lead, data)` helper coordinates re-rendering on retry success. |
| `crm-leads-detail-messages.js` | 150 | **[P31 0b + 6]** Sibling module owning the messages tab subview. **[0b]** `fetchMessages(leadId, tenantId)` + `renderMessagesList(messages)` extracted verbatim from detail.js. **[6]** New: `getFailedMessages` + `renderFailedSection` (red-bordered collapsible with channel icon, Hebrew template name, translated error reason via CrmMessageErrorLabels, timestamp, "🔄 נסה שוב" retry button per row) + `wireFailedRetryHandlers` (calls CrmMessaging.sendMessage with stripped base slug + original event_id + run_id; on success drops in-memory row + refreshes registered-tab failed counts). |
| `crm-leads-tab-filters.js` | 104 | **[P31 0a]** Sibling module owning the filter-chip + pagination chrome of the leads table. Verbatim extraction from crm-leads-tab.js. `renderChipsBar(host, opts)` and `renderPaginationBar(box, opts)` — parameterized via opts callbacks (onClearChip / onPageChange / onLoadMore) so the parent retains its private state without leaking. |
| `crm-message-error-labels.js` | 56 | **[P31 4]** Hebrew translation map for `crm_message_log.error_message` codes. `errorLabel(rawError)` → exact-match (8 codes) then prefix-match (6 prefixes) then raw fallback. Covers `missing_required_variable`, `phone_not_allowed`, `template_not_found`, `payment_link_missing_or_mismatch`, `recipient_invalid`, `make_webhook_*`, `make_call_exception`. |
...
| `crm-event-day-manage.js` | 278 | **[B8 + P23]** Manage table (Tailwind) + arrived column widget (waiting-to-purchase + purchased sections with amount badges) + running-total bar + purchase amount modal with 3xl tabular-nums input. **[P23]** `couponCell` and `toggleCoupon` extracted to `crm-event-day-coupon.js` (Step 0 refactor); cancel button "בטל" wired into the coupon column via `CrmAttendeeCancel.cancelButtonHtml`. `filterRows` now hides `status='cancelled'` by default (admin can still see them by selecting "ביטל" in the status filter). `refreshAttendeeRow` SELECT extended with `status, cancelled_at` so cancellations reflect locally. |
| `crm-event-day-coupon.js` | 140 | **[P23 Step 0 + 0.5]** New module extracted from `crm-event-day-manage.js`. Owns `couponCell(r, ctx)` and `toggleCoupon(id, btn, ctx)`. Includes lifecycle guards: blocks dispatch when event status not in {`registration_open`, `invite_new`, `waiting_list`, `2_3d_before`, `event_day`, `invite_waiting_list`} or attendee status not in {`registered`, `quick_registration`, `manual_registration`, `confirmed`, `attended`, `invited`}. ctx callbacks `renderTable` + `updateLocal` keep the manage module in charge of rendering. Local helpers `couponToast` / `couponLog` uniquely named to avoid Rule 21 collision with same-purpose helpers in adjacent files. Exports `window.CrmEventDayCoupon`. |
| `crm-attendee-cancel.js` | 142 | **[P23 + P23.1]** Attendee cancellation dialog. Three paths chosen by `payment_status`: `paid` opens a 2-button choice modal ("מגיע החזר" → status='cancelled' + payment_status='refund_requested' with timestamps; "לא מגיע החזר" → **no_refund_due_marked=true + no_refund_due_marked_at=now()** [P23.1: was the broken `payment_status='no_refund_due'` enum write that the CHECK constraint rejected]; coupon stays unfreed); everything else opens a simple confirm modal that sets status='cancelled' + cancelled_at=now(). Every UPDATE filters by tenant_id (Rule 22). `cancelButtonHtml(attendee)` returns empty string for already-cancelled attendees so the button auto-hides. `openCancelDialog(attendeeId, opts)` accepts `opts.onAfterCancel(attendeeId)` for caller-driven row refresh. Local helpers `_cancelToast` / `_logCancel` uniquely named. Log action label `crm.attendee.mark_no_refund_due_flag` (renamed from `mark_no_refund_due` in P23.1). Exports `window.CrmAttendeeCancel`. |
...
| `crm-automation-engine.js` | 326 | **[P8 + P20 + P21 + EVENT_CONFIRMATION_EMAIL + CRM_HOTFIXES + CRM_PRE_MERGE + AUTOMATION_ENGINE_SPLIT]** Rule evaluation engine. `CrmAutomation.evaluate(triggerType, data)` loads matching `crm_automation_rules`, evaluates conditions (always/status_equals/count_threshold/source_equals), resolves recipients, builds `sendPlan`. **[P20]** Shows `CrmConfirmSend` modal if loaded; falls back via `CrmAutomationDispatch.dispatchPlanDirect` (P20 path was extracted into `crm-automation-dispatch.js` 2026-05-01 for Iron Rule 12 headroom). **[P21]** `resolveRecipients` accepts optional 4th param `actionConfig`; when `action_config.recipient_status_filter` is set and recipient_type is tier2*, filters the `crm_leads.status IN (...)` list to the chosen statuses. **[EVENT_CONFIRMATION_EMAIL]** `buildVariables` composes `%lead_id%` / `%event_id%` for the confirmation email QR code. **[CRM_HOTFIXES]** `promoteWaitingLeadsToInvited` lives in `crm-automation-post-actions.js`; wired from this engine + the dispatch path. **[CRM_PRE_MERGE]** `buildVariables` now injects `vars.lead_id = lead.id` so `%lead_id%` resolves on the UI-register path (fixes the broken QR where the literal `%lead_id%` was encoded). |
| `crm-automation-dispatch.js` | 52 | **[AUTOMATION_ENGINE_SPLIT, 2026-05-01]** P20 fallback dispatch path. `window.CrmAutomationDispatch.dispatchPlanDirect(items)` — direct dispatch when `CrmConfirmSend` isn't loaded (server-side / non-UI callers). Body byte-identical to the original `dispatchPlanDirect` that used to live in the engine. Calls `CrmMessaging.sendMessage` per item, calls `CrmAutomationRuns.stampLog` per ok-with-runId, then runs `CrmAutomationPostActions.promoteWaitingLeadsToInvited`. Pure dependencies on window globals; no engine closure state. |
| `crm-status-color-settings.js` | 120 | **[PRE_CUTOVER_QA_C B10, 2026-05-01]** Tenant-wide event-status color customization. `window.CrmStatusColorSettings.open()` shows a Modal listing every active row of `crm_statuses WHERE entity_type='event'` with a native `<input type="color">` per status. Save batches `UPDATE crm_statuses.color` for each changed row, invalidates `window.CRM_STATUSES._loaded`, calls `CrmHelpers.loadStatusCache()` to repopulate, then `window.reloadCrmEventsTab()` so the events-table badges pick up the new colors without a page reload. No DDL, no external library. Wired to the ⚙️ button in the events-tab toolbar. |
| `crm-automation-history.js` | 213 | **[OVERNIGHT_M4_SCALE_AND_UI Phase 7 + P29]** Automation Execution History tab. `renderAutomationHistory(host)` shows the last 50 `crm_automation_runs` rows with derived counts via `loadRunCounts` (GROUP BY status WHERE run_id IN ...). Row click → `openDrillDown(runId, tenantId)` modal. **[P29]** Drill-down now fetches the run row (rule_name, status, started_at, finished_at, updated_at, error_message, total_recipients) in parallel with message_log rows; renders `renderRunHeader(run)` (status badge + timestamps + planned-recipients + red error banner) above the body, then state-aware empty-states via `renderEmptyState(run)` for runs with zero log rows: aborted → "ריצה הופסקה אוטומטית - חלון האישור פג", running → "ריצה ממתינה לאישור ידני... בעוד {N} דקות" (computed from `updated_at`), other → "אין הודעות לריצה זו". Existing table + retry-failed button preserved when rows exist. Iron Rule 22: every SELECT carries `.eq('tenant_id', tenantId)`. |
| `crm-confirm-send.js` | 302 | **[P20 + P21 + P29 + ATOMIC_CONFIRMATION_FLOW]** Confirmation Gate shown before any automated send. **[P21]** 2-tab layout: Messages tab (1 card per channel per rule with representative body preview) + Recipients tab (sortable paginated table, 50/page, dedup by lead_id with ×N badge). **[P29]** `writePendingReviewRows` includes `run_id: it.run_id || null` in the inserted row so abandoned runs link drill-down ↔ pending_review. **[ATOMIC_CONFIRMATION_FLOW, 2026-05-04]** Modal contract is now a 3-button atomic commit: **Cancel** (`#ccs-cancel`, no callback — no commit, no side effects), **Confirm without notify** (`#ccs-confirm-no-notify` → `onChoice({dispatch:false}, sendPlan)`, status changes only), **Confirm and notify** (`#ccs-confirm-notify` → `onChoice({dispatch:true}, sendPlan)`, status + dispatches messages). `show(sendPlan, onChoice)` signature replaces the old `(sendPlan, onApprove)`. Pairs with the `automation-engine` EF gating post-actions/queue_send on `mode === 'dispatch'` so cancel never leaks state. |
| `crm-automation-client.js` | 130 | **[ATOMIC_CONFIRMATION_FLOW, 2026-05-04]** Server-side automation client — only browser path that talks to the `automation-engine` EF. `window.CrmAutomationClient.evaluate(triggerType, triggerData, onAfterConfirm?)`: POSTs `mode='evaluate'` → renders `CrmConfirmSend.show` modal → on user choice POSTs `mode='dispatch'` with `plan_items` + `dispatch_messages` from the 3-button modal. Optional 3rd-arg `onAfterConfirm` callback fires AFTER dispatch resolves (modal-stack-race fix from B.3 — caller cleanup chains like `modal.close + reloadDetail` defer until user resolves the confirmation modal, preventing the global `Modal.close()` from popping the confirmation off the stack before the user can click). Symmetric on the no-modal Step 4 fallback path. Used by `crm-attendee-move.js` + `crm-event-register.js`; the 3 fire-and-forget callsites (`crm-event-actions.js`, `crm-lead-actions.js` ×2) call without `onAfterConfirm` since they don't have the await-then-sync-cleanup race shape. |
...
| `automation-engine` | `index.ts` + `engine.ts` + `prepare-plan.ts` + `dispatch.ts` + `deno.json` | 104 + 224 + 182 + 81 | M4_AUTOMATION_ENGINE_SERVER_SIDE + ATOMIC_CONFIRMATION_FLOW | Server-side automation engine that ports the browser engine logic into an EF. Two modes: `mode='evaluate'` (load matching `crm_automation_rules`, evaluate conditions, resolve recipients, return `plan_items` + `run_id` — no DB-mutating side effects) and `mode='dispatch'` (re-evaluate or honor caller's `plan_items`, run post-actions + queue_send writes, dispatch via `send-message` if `dispatch_messages !== false`). The `mode === 'dispatch'` gate at engine.ts:159 wraps the post-action loop so cancel-from-modal never leaks state (ATOMIC_CONFIRMATION_FLOW Bug 1). Cron callers (`event_day_status_flip`, `event_2_3d_before_status_flip`) POST `mode='dispatch'` without `plan_items` and inherit `dispatch_messages=true` default → zero behavior change. Browser callers go through `crm-automation-client.js`. `verify_jwt: true`. Live live state at SPEC close (2026-05-04): version 7 ACTIVE. |

### Edge Function shared helpers — `supabase/functions/_shared/`
| File | Function(s) | Lines | Purpose |
|------|-------------|-------|---------|
| `tenant-config.ts` | `loadTenantConfig(db, tenantId): Promise<TenantConfig \| null>` | 73 | Single SELECT against `tenants` returning a typed `TenantConfig` (storefront_url, whatsapp_phone_e164, support_phone_display, business_phone, business_address, brand{gold/_light/_hover}, ui_config). Caller-decides-fallback: returns null fields when a key is missing rather than substituting a hardcoded default. Replaces previously-hardcoded `STOREFRONT_URL`, WhatsApp number, etc. across `quick-register`, `send-message/url-builders`, `resolve-link`. Added by M4_HARDCODED_PRIZMA_REMOVAL (2026-05-06). |
