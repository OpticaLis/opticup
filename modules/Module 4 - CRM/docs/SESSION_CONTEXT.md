...
> **Current phase:** Go-Live P3c+P4 (Messaging Pipeline — Edge Function + Trigger Wiring) — ✅ CLOSED
> **Next phase:** P5 (message content authoring) and/or follow-up trigger wiring (events/reminders/broadcast)
> **Branch:** develop
...
- Messaging Hub dispatch pipeline now **architecturally v3** (P3c+P4 ✅, 2026-04-22). Client → Supabase Edge Function `send-message` → Make webhook (Scenario `9104395`, 4 modules: Webhook → Router → Global SMS | Gmail). All business logic lives in the Edge Function; Make is a send-only pipe. Lead-intake trigger wiring live: new-lead and duplicate paths both dispatch SMS + Email via `lead_intake_new` / `lead_intake_duplicate` templates. Event / attendee / broadcast / reminder triggers deferred to P5+ (content first, scheduler second).
...
**Full roadmap:** ~~P1~~ ✅ → ~~P2a~~ ✅ → ~~P2b~~ ✅ → ~~P3a~~ ✅ → ~~P3b~~ ✅ → ~~P3c+P4~~ ✅ → P5 (message content authoring) → P6 (full demo test) → P7 (Prizma cutover). See `modules/Module 4 - CRM/go-live/ROADMAP.md`.
