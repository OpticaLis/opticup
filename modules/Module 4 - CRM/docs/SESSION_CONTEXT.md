...
> **Current phase:** Go-Live P3b (Make Message Dispatcher) — 🟡 PARTIAL (scenario built, runtime blocked on Make UI step)
> **Next phase:** P3b completion — then P4 (connect CRM→Make triggers)
> **Branch:** develop
...
| Go-Live P3b — Make Message Dispatcher | 🟡 PARTIAL | Make scenario `9103817` "Optic Up — Send Message" built via Make MCP API in Demo folder (499779). 13 modules: webhook → SetVariable FullSlug → SetVariable NormalizedPhone → HTTP GET template → Router (3 routes: SMS/Email/error). Uses Global SMS connection 13198122 and Gmail 13196610. Blueprint accepted by Make (`isinvalid` false at creation, structurally valid). Webhook `4068400` URL `https://hook.eu2.make.com/b56ocktlm8rcpj52pu12qkthpke71c77`. Two new CRM files: `modules/crm/crm-messaging-config.js` (6 lines — webhook URL) and `modules/crm/crm-messaging-send.js` (52 lines — `CrmMessaging.sendMessage()` helper). Added 2 `<script>` tags to `crm.html`. Reference doc `modules/Module 4 - CRM/go-live/make-send-message.md`. Tests 1 ✅ (webhook 200 Accepted), 2 ✅ (browser console: `typeof CrmMessaging.sendMessage === 'function'`), 3 ✅ (dry-run from browser returned `{ok:true}`). **BLOCKED:** every scenario execution fails with `BundleValidationError: Validation failed for 4 parameter(s)` because API-created webhooks have no registered data structure — references like `{{1.tenant_id}}` and `{{1.variables.email}}` cannot be validated at design time. Resolution requires UI-only steps Daniel must perform (see FINDINGS M4-MAKE-01). Tests 4, 5, 6 (live SMS/Email/error-path) cannot run until Daniel (a) opens the webhook module in Make UI and clicks "Re-determine data structure" while firing a sample POST, and (b) replaces `REPLACE_WITH_SERVICE_ROLE_KEY` in all 4 HTTP modules with the real service_role key. Scenario left DEACTIVATED after the session to avoid further queued-payload errors. No DB changes. Criteria passed: 1, 2, 3, 8, 9, 11 (partially — 12 docs are partial, 13 N/A since no test rows inserted). |
...
**Go-Live P3b completion (requires Daniel's Make UI action):**
1. Open Make → team 402680 → Demo folder → scenario `9103817` "Optic Up — Send Message".
2. Click the webhook trigger module → "Re-determine data structure" → send a sample POST to `https://hook.eu2.make.com/b56ocktlm8rcpj52pu12qkthpke71c77` with the full payload shape (example body in `modules/Module 4 - CRM/go-live/make-send-message.md` §Webhook payload) → save. This registers the payload schema so references like `{{1.tenant_id}}` validate.
3. In each of the 4 HTTP modules (`id` 4 template-fetch, `id` 8 SMS-log, `id` 12 Email-log, `id` 13 error-log), replace `REPLACE_WITH_SERVICE_ROLE_KEY` in the Authorization header with the real Supabase service_role key.
4. Reactivate the scenario.
5. Tell the next Claude Code session "P3b UI steps done" — it will run Tests 4, 5, 6, 7 from the SPEC and close P3b.

**Full roadmap:** ~~P1~~ ✅ → ~~P2a~~ ✅ → ~~P2b~~ ✅ → ~~P3a~~ ✅ → P3b 🟡 (scenario built, awaiting Daniel UI step) → P4 (connect CRM→Make triggers) → P5 (message content) → P6 (full demo test) → P7 (Prizma cutover). See `modules/Module 4 - CRM/go-live/ROADMAP.md`.

**P3a follow-ups (not blockers):**
- Terms approval mechanism (send link via WhatsApp/SMS → flip `terms_approved`) — scheduled for P3b.
