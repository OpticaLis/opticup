...
4. **MEDIUM — Lead-status taxonomy has dead slugs and overlapping semantics.** `crm_statuses` declares 13 lead statuses including BOTH `waiting` and `waitlist`; only `waiting` is in use (1 lead in Prizma). `confirmed` and `confirmed_verified` differ ambiguously. 1137/1235 visible Prizma leads (92%) sit in `invited` — the lifecycle has flattened. **[2026-05-14 UPDATE — RESOLVED:** the flat-`invited` mass was the result of past closed/completed events never recycling their leads. Daniel-approved sweep (`M4_STALE_INVITED_LEADS_SWEEP`, 2026-05-14) flipped 1042 leads `invited` → `waiting`; the event-close auto-recycle behavior shipped by `M4_WAITLIST_SYNC_PRIORITY_FIX` (2026-05-14) prevents recurrence. Live state 2026-05-14: 0 leads at `invited`, 1136 at `waiting` (correctly between events). The `waiting` cluster is operationally correct. `confirmed_verified` deprecation remains deferred until "purchaser" status is designed (M4_REMOVE_CONFIRMED_VERIFIED escalated 2026-05-13).**
