-- Post-migration capture of public.register_lead_to_event via pg_get_functiondef.
-- Captured 2026-05-14 after applying migration register_lead_to_event_return_shape_fix.
-- Body md5 = 31fea2eaf0086cf917d0d65a8595d41c, length = 4674.
-- Pre-migration baseline (yesterday's M4_REGISTER_LEAD_TO_EVENT_RPC_MAP): md5=dbd2ccd1eb068b494edfec5cf7788563, length=4603.
-- Delta = +71 bytes. The only semantic change is the fresh-INSERT over-capacity
-- RETURN clause now uses CASE WHEN instead of hardcoded 'waiting_list'.
