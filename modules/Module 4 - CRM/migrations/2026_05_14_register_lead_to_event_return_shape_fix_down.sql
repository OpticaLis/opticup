-- 2026_05_14_register_lead_to_event_return_shape_fix_down.sql
-- Paired rollback for 2026_05_14_register_lead_to_event_return_shape_fix_up.sql.
-- Restores the RPC body verbatim to the pre-migration snapshot
-- (md5=dbd2ccd1eb068b494edfec5cf7788563, length=4603, captured by SPEC
-- M4_REGISTER_LEAD_TO_EVENT_RPC_MAP on 2026-05-14).
--
-- Reverts the fresh-INSERT over-capacity branch's RETURN to the hardcoded
-- 'waiting_list' literal that FIND-1 documents as buggy. Apply only if the
-- forward migration must be rolled back; no other use case.
