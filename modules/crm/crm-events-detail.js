...
(function () {
  ...
  async function fetchDetail(eventId) {
    ...
    var attQ = sb.from('v_crm_event_attendees_full')
      .select('id, lead_id, event_id, full_name, phone, email, status, status_name, status_color, purchase_amount, checked_in_at, registered_at, cancelled_at, coupon_sent, payment_status, paid_at, scheduled_time, no_refund_due_marked, no_refund_due_marked_at')
    ...
    if (noShow.length) {
      ...
      var rows = noShow.map(function (a) {
        return '<tr><td class="px-3 py-2 text-slate-800">' + escapeHtml(a.full_name || '') + '</td>' +
          ...
          '<td class="px-3 py-2 text-center">' + (window.CrmPayment ? CrmPayment.renderStatusPill(a.payment_status) + CrmPayment.renderNoRefundDueChip(a) : '—') + '</td><td class="px-3 py-2 text-center">' + yes + '</td></tr>';
      }).join('');
  ...
  function renderAttendeesGrouped(attendees) {
    ...
    Object.keys(groups).forEach(function (slug) {
      ...
      groups[slug].slice(0, 40).forEach(function (a) {
        ...
        var fee = (window.CrmPayment ? ' ' + CrmPayment.renderStatusPill(a.payment_status) + CrmPayment.renderNoRefundDueChip(a) : '') + ((a.payment_status === 'paid') ? ' <span class="inline-block text-xs bg-emerald-100 text-emerald-700 font-semibold px-1.5 py-0.5 rounded" title="פיקדון שולם">💰</span>' : '');
