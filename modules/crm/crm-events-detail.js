    ...
    if (noShow.length) {
      ...
      var rows = noShow.map(function (a) {
        return '<tr><td class="px-3 py-2 text-slate-800">' + escapeHtml(a.full_name || '') + '</td>' +
          ...
          '<td class="px-3 py-2 text-center">' + (window.CrmPayment ? CrmPayment.renderStatusPill(a.payment_status) + CrmPayment.renderNoRefundDueChip(a) + CrmPayment.renderCreditIndicator(a) : '—') + '</td><td class="px-3 py-2 text-center">' + yes + '</td></tr>';
      }).join('');
