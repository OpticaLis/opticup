...
(function () {
  ...
  // Messages-tab fetch + render in crm-leads-detail-messages.js (P31 0b).
  ...
  async function fetchDetailData(leadId) {
    ...
    // Per-lead message history fetched by sibling module (P31 commit 0b).
    var msgsP = window.CrmLeadsDetailMessages
      ? CrmLeadsDetailMessages.fetchMessages(leadId, tid)
      : Promise.resolve([]);

    var r = await Promise.all([notesQ, histQ, msgsP]);
    if (r[0].error) throw new Error('notes: ' + r[0].error.message);
    if (r[1].error) throw new Error('history: ' + r[1].error.message);
    return { notes: r[0].data || [], history: (r[1].data && r[1].data[0]) || null, messages: r[2] || [] };
  }
  ...
  function renderMessages(messages) {
    if (!window.CrmLeadsDetailMessages) return '';
    return CrmLeadsDetailMessages.renderMessagesList(messages || []);
  }
  ...
  function row(label, value, extra) {
    return '<div class="' + CLS_DETAIL_ROW + '">' +
      ...
      '<span class="text-sm text-slate-900">' + escapeHtml(value == null ? '' : value) + (extra || '') + '</span>' +
    '</div>';
  }
  ...
  function wireTabs(body, lead, data) {
    body.querySelectorAll('[data-detail-tab]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (key === 'details' && window.CrmLeadActions && CrmLeadActions.wireDetailsButtons) { var refresh = function () { host.innerHTML = renderTabContent('details', lead, data.notes, data.history, data.messages); CrmLeadActions.wireDetailsButtons(host, lead, refresh); }; CrmLeadActions.wireDetailsButtons(host, lead, refresh); }
      });
    });
  }
