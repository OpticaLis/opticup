// lens-inventory-modals.js — REAL ➕➖ wiring
...
(function () {
  ...
  // Per-tenant cache of stock_adjustment_reason rows (direction=-1, is_active=true)
  let _reasonsCache = null;
  async function loadAdjustmentReasons() {
    if (_reasonsCache) return _reasonsCache;
    const tid = getTenantId();
    const { data, error } = await sb.from('stock_adjustment_reason')
      .select('id, code, name_he, direction, sort_order')
      .eq('tenant_id', tid)
      .eq('is_active', true)
      .eq('direction', -1)
      .order('sort_order');
    if (error) { console.error('[lens-inv-modals] failed to load adjustment reasons', error); return []; }
    _reasonsCache = data || [];
    return _reasonsCache;
  }

  async function openQtyConfirmModal(variantId, sph, cyl, lot, emp) {
    const max = lot.qty_remaining || 0;
    const sphLabel = sph !== '' && sph !== null ? sph : '—';
    const cylLabel = cyl !== '' && cyl !== null ? cyl : '—';
    const reasons = await loadAdjustmentReasons();
    if (reasons.length === 0) {
      if (window.Toast) Toast.error('אין סיבות התאמה זמינות (stock_adjustment_reason ריק)');
      return;
    }
    const reasonOptions = reasons.map(function (r) {
      return '<option value="' + escapeHtmlSafe(r.id) + '">' + escapeHtmlSafe(r.name_he) + '</option>';
    }).join('');
    const body = '<div style="padding:8px 4px; font-size:13px;">' +
      ...
      '</div>' +
      '<label style="font-size:12px; color:#475569;">סיבת ההתאמה</label>' +
      '<select id="adjust-reason-id" style="width:100%; padding:8px 10px; border:1px solid #cbd5e1; border-radius:5px; margin-top:4px; font-size:14px;">' + reasonOptions + '</select>' +
      '<label style="font-size:12px; color:#475569; margin-top:10px; display:block;">כמות להפחתה (מקסימום ' + max + ')</label>' +
      ...
      '<label style="font-size:12px; color:#475569; margin-top:10px; display:block;">הערות (אופציונלי)</label>' +
      '<textarea id="adjust-notes" rows="2" style="width:100%; padding:6px 10px; border:1px solid #cbd5e1; border-radius:5px; margin-top:4px;" placeholder="פירוט נוסף..."></textarea>' +
    ...
    overlay.querySelector('#adjust-confirm').addEventListener('click', async function () {
      ...
      const reasonId = overlay.querySelector('#adjust-reason-id').value;
      const notes = (overlay.querySelector('#adjust-notes').value || '').trim() || null;
      ...
      if (!reasonId) { err.textContent = 'יש לבחור סיבת התאמה'; err.style.display = 'block'; return; }
      try {
        const tid = getTenantId();
        ...
        modal.close();
        if (window.Toast) Toast.success('מלאי עודכן (-' + qty + ' יח׳)');
        // Iron Rule 2: writeLog audit
        if (typeof writeLog === 'function') {
          writeLog('lens.inventory.adjustment_lost', null, { variant_id: variantId, lot_id: lot.id, qty: qty, reason_id: reasonId, adjustment_id: data, performed_by: emp.id });
        }
        // Refresh grid + lots panel
        ...
        console.error('[lens-inv-modals] record_adjustment_lost failed', e);
      }
    });
  }
