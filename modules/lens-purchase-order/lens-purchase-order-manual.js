...
// Opens a Modal with description + qty + unit_cost, pushes a new line into LensPO.lines.
...
(function () {
  ...
  function openAddManualModal() {
    ...
    const body = '<div style="display:flex;flex-direction:column;gap:10px;padding:8px 4px;font-size:13px;">' +
      '<div><label style="font-size:11px;color:#5d6d7e;">תיאור פריט (חובה)</label><input type="text" id="manual-desc" style="width:100%;padding:6px 10px;border:1px solid #d0d4d9;border-radius:5px;" placeholder="למשל: דוגמית בונוס מהספק"></div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">' +
        '<div><label style="font-size:11px;color:#5d6d7e;">כמות</label><input type="number" id="manual-qty" min="1" value="1" style="width:100%;padding:6px 10px;border:1px solid #d0d4d9;border-radius:5px;"></div>' +
        '<div><label style="font-size:11px;color:#5d6d7e;">מחיר יחידה (' + escapeHtmlSafe(currency) + ')</label><input type="number" id="manual-cost" min="0" step="0.01" value="0" style="width:100%;padding:6px 10px;border:1px solid #d0d4d9;border-radius:5px;"></div>' +
      '</div>' +
