...
| 2 | P1.1 | M3_UTM_TRIPLE_LAYER_PERSISTENCE | 2 | cross-cut (M3 storefront + M4 lead-intake EF + DB schema) — Architect to scope ownership at SPEC-author time | 4-6 hrs | PLANNED |
| 3 | P1.2 | M4_BROADCAST_ID_PROPAGATION | 5 | opticup-strategic (M4) | 3-4 hrs | PLANNED |
...
|---|---|---|---|
| E1 | Multi-Touch Attribution (MTA) Engine | Distribute conversion credit across 5+ touchpoints per customer journey via configurable models (linear / time-decay / U-shape / data-driven) | Phase 1 P1.1: do NOT lock first-touch as the only model. Store every UTM hit per lead as a separate event row (e.g. `crm_lead_touchpoints` table or structured event log) — not just first-touch columns on `crm_leads`. First-touch becomes one view over the touchpoint log, not the only truth. |
| E2 | Predictive LTV → CAC per channel | ML model predicts customer lifetime value per acquisition source; budget decisions follow LTV/CAC ratios per channel | Phase 1+2: tag every revenue event (order, redemption, repeat purchase) with the originating acquisition source (broadcast_id + UTM bag + first-touch + last-touch). M5/M7/M13 SPECs must persist these tags on every revenue row so the model has training data later. |
...
| E7 | Customer Journey Analytics | Visualize the full path: impression → click → landing → scroll-depth → form → submit → cart → purchase, per cohort | Phase 1 P1.1 + P1.4: structured event log for every funnel touchpoint, not just creation/status flips. Even if dashboards are deferred, the raw events must be captured. Adding events later is cheap; backfilling missing events is impossible. |
