...
**Last updated:** 2026-05-09 (after REC-SITE-018 close — `storefront_config.custom_domain` flipped apex→www for prizma; OG/canonical/Twitter/hreflang verified live)
...
| REC-SITE-017 | MEDIUM | `M3_SITEMAP_BRAND_404_CLEANUP` — sitemap-dynamic.xml emits `/brands/{slug}/` for every row in `v_storefront_brands` but ~30-40% (West Coast, Gipsy Kids, Caroline DK, etc.) have no public detail page → 404. Filter brand-slug emission to only brands with backing public detail pages. Source: M3_SITEMAP_CONSOLIDATION/FINDINGS.md M3-DATA-01. | 1 SPEC, 30-60 min. |
