-- M4_STATIC_SHORT_LINKS_EDIT_DELETE_AND_LABEL (Sprint 3 Item 5, re-applied 2026-05-22).
-- Mirrors what was applied via Supabase MCP apply_migration on 2026-05-22.
-- The original migration (20260521211000) failed silently during a Supabase
-- outage; this is the successful re-apply.
--
-- Iron Rule 32 destructive ops:
-- 1. ALTER TABLE ADD COLUMN (additive, NULL default, idempotent).
-- 2. 3 CREATE OR REPLACE FUNCTION (1 extends signature + 2 new).
-- 3. The delete RPC, when called, HARD-DELETEs the short_links row + its
--    short_link_clicks children, gated to link_type='template_static' only
--    + tenant_id scoped.

ALTER TABLE public.short_links ADD COLUMN IF NOT EXISTS label text NULL;
COMMENT ON COLUMN public.short_links.label IS
  'Operator-facing nickname for self-serve static short links. NULL for system rows.';

CREATE OR REPLACE FUNCTION public.crm_create_static_short_link(
  p_tenant_id uuid, p_target_url text, p_label text DEFAULT NULL
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public' AS $$
DECLARE
  v_jwt_tenant uuid; v_code text; v_retries int := 0; v_new_id uuid;
  v_target_clean text; v_label_clean text;
BEGIN
  ...
  VALUES (
    ...
    '2099-12-31 23:59:59+00'::timestamptz, NULL, NULL, NULL, NULL, v_label_clean
  )
  RETURNING id INTO v_new_id;
  RETURN jsonb_build_object('ok', true, 'id', v_new_id, 'code', v_code,
    'target_url', v_target_clean, 'short_path', '/r/' || v_code, 'label', v_label_clean);
END; $$;
...
CREATE OR REPLACE FUNCTION public.crm_update_static_short_link(
  p_tenant_id uuid, p_link_id uuid, p_target_url text, p_label text DEFAULT NULL
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public' AS $$
DECLARE v_jwt_tenant uuid; v_target_clean text; v_label_clean text; v_existing record;
BEGIN
  BEGIN v_jwt_tenant := (current_setting('request.jwt.claims', true)::json ->> 'tenant_id')::uuid;
  EXCEPTION WHEN OTHERS THEN v_jwt_tenant := NULL;
  END;
  ...
  IF v_existing.link_type <> 'template_static' THEN
    RETURN jsonb_build_object('ok', false, 'error', 'wrong_link_type');
  END IF;
  UPDATE short_links SET target_url = v_target_clean, label = v_label_clean
   WHERE id = p_link_id AND tenant_id = p_tenant_id;
...
CREATE OR REPLACE FUNCTION public.crm_delete_static_short_link(p_tenant_id uuid, p_link_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public' AS $$
DECLARE v_jwt_tenant uuid; v_existing record; v_clicks_deleted int := 0;
BEGIN
  BEGIN v_jwt_tenant := (current_setting('request.jwt.claims', true)::json ->> 'tenant_id')::uuid;
  EXCEPTION WHEN OTHERS THEN v_jwt_tenant := NULL;
  END;
